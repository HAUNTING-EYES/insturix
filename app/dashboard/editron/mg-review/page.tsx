...
export default function MgReviewPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl p-6 font-sans">
      ...
      <button onClick={() => void refresh()} className="mb-6 rounded border px-3 py-1 text-sm hover:bg-neutral-100">↻ refresh</button>
      ...
      <div className="grid gap-4">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1">
                    {REASONS.map((reason) => {
                      ...
                      return (
                        ...
                        }} className={`rounded-full border px-2 py-0.5 text-xs ${on ? 'bg-blue-600 text-white' : 'bg-white'}`}>
                          {reason}
                        </button>
                      );
                    })}
        ...
        {items.length === 0 ? <p className="text-neutral-400">No corpus items. Seed `.calibration-temp/mg-eval-seed.jsonl` and reload.</p> : null}
      </div>

      {ready ? (
        <p className="mt-6 rounded bg-emerald-50 p-3 text-sm text-emerald-700">
          {labeled} labeled — run <code className="rounded bg-white px-1">npx tsx scripts/prompt-optimization/eval-mg-calibrate.ts --labels=.calibration-temp/mg-eval-labeled.jsonl</code> to produce the calibration artifact.
        </p>
      ) : null}
    </main>
  );
