...
export async function POST(request: Request) {
  ...
  if (!serviceSecret || authHeader !== `Bearer ${serviceSecret}`) {
    console.warn('Unauthorized webhook call for Musitron');
  ...
  try {
    ...
    // 3. Process refund
    await processRefund('musitron', taskType, userId);

    return new NextResponse('Refund processed successfully', { status: 200 });
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    console.error('Error processing Musitron refund webhook', { errorMessage });
