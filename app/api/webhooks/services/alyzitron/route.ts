...
import { getCollections } from '../../../../api/services/alyzitron/utils/mongodb';
import { logger } from '../../../../api/services/alyzitron/utils/logger';

export async function POST(request: Request) {
  // 1. Authenticate the request
  const authHeader = request.headers.get('authorization');
  const serviceSecret = process.env.ALYZITRON_SERVICE_SECRET;
  ...
  const { analysisId, error } = body; // Changed taskId to analysisId
  ...
  try {
    ...
    const task = await analyses.findOne({ _id: analysisId }); // Changed taskId to analysisId
 
    if (!task) {
      logger.warn('Webhook received for non-existent Alyzitron task', { analysisId }); // Changed taskId to analysisId
      return new NextResponse('Task not found', { status: 404 });
    }
 
    const userId = task.clerkUserId;
    if (!userId) {
        logger.error('Could not find clerkUserId on Alyzitron task for failure handling', { analysisId }); // Changed taskId to analysisId
    ...
    await handleTaskFailure({
      taskId: analysisId, // Changed taskId to analysisId
    ...
    logger.error('Error processing Alyzitron failure webhook', {
      analysisId, // Changed taskId to analysisId
      errorMessage,
    });
