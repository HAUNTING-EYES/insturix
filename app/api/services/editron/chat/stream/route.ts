...
export async function POST(req: NextRequest) {
  try {
    ...
    // Generate project summary for smart context — use cache to avoid recomputing
    // on every message when the project hasn't been modified.
    // OLD: const summary = generateProjectSummary(project);
    //      const contextMessage = formatSummaryForPrompt(summary);
    const contextMessage = getCachedProjectContext(project);
