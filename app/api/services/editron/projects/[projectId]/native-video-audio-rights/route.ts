...
interface RouteDependencies {
  ...
  attest: typeof reattestNativeVideoAudioRights;
}

const defaultDependencies: RouteDependencies = {
  authenticate: auth,
  attest: reattestNativeVideoAudioRights,
};
...
export async function POST(
  ...
  try {
    ...
    if (error instanceof NativeVideoAudioRightsAttestationError) {
      return NextResponse.json(
        { success: false, error: error.message, code: error.code },
        { status: error.httpStatus },
      );
    }
    console.error('[NativeVideoAudioRightsAttestationRoute] Unexpected failure', error);
    return NextResponse.json(
      {
        success: false,
