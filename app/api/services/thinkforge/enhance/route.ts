...
import { ThinkForgePromptEnhancementRequestSchema } from '@/lib/thinkforge/schemas/prompt-enhancement';
...
export async function POST(req: NextRequest) {
    ...
    const parsedRequest = ThinkForgePromptEnhancementRequestSchema.safeParse(requestInput);
    if (!parsedRequest.success) {
        return NextResponse.json({
            error: 'Invalid prompt enhancement request',
            code: 'invalid_prompt_enhancement_request',
            details: parsedRequest.error.flatten(),
        }, { status: 422 });
    }
    const { prompt, authoringRequest } = parsedRequest.data;
    let enhancementPolicy: string;
    try {
        enhancementPolicy = describeThinkForgePromptEnhancementPolicy(authoringRequest);
    } catch {
        return NextResponse.json({
            error: 'Unsupported authoring destination',
            code: 'unsupported_prompt_enhancement_destination',
        }, { status: 422 });
    }
...
<artifact_contract>${enhancementPolicy}</artifact_contract>
<rules>
