...
export async function POST(request: Request) {
  ...
  console.log('Bypass mode:', isDevelopmentBypass ? '✅ DEVELOPMENT (bypassing QStash)' : '🚀 PRODUCTION (verifying QStash)');
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        await AlyzitronRTDBManager.updateTaskStatus(userId, taskId, 'completed');
        ...
        if (vertexError instanceof Error && vertexError.message.includes('Cannot find module')) {
      ...
      // Refund credits
      const minutes = task.usageMinutes || Math.ceil((task.metadata?.videoDuration || 0) / 60);
