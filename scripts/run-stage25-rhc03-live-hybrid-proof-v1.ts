...
const environmentPath = process.env.EDITRON_RHC03_SANDBOX_ENV_FILE?.trim()
...
const executionId = `rhc03-live-${createdAt.replace(/[-:.TZ]/g, '')}`;
const outputDirectory = path.resolve(
  repoRoot,
  '.calibration-temp',
  'open-ended-planner-v2',
  'stage25-rhc03-live-render-v1',
  executionId,
);
...
console.log(JSON.stringify({
  ...
  workerImplementationSha256:
    result.receipt.generatedProgram.workerImplementationSha256,
  sourceSynchronization: result.receipt.synchronizationProof.disposition,
  renderedVisualDisposition:
    result.receipt.renderedVisualProof.technicalDisposition,
...
function required(name: string): string {
  ...
  if (!value) throw new Error(`RHC03 live hybrid proof missing ${name}`);
  return value;
}
