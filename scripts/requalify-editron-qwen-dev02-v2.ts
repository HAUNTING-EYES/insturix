...
import { buildConnectedStage1RequalificationHandoffV2 } from '../lib/editron/research/open-ended-planner/development-connected-stage1-requalification-v2';
...
const DEFAULT_SOURCE = '.calibration-temp/open-ended-planner-v2/cohort-runs/'
  + 'qwen-continuation-20260816184825/dev-02-continuation-receipt.json';
...
async function main(): Promise<void> {
  ...
  const runId = `qwen-dev02-requalification-${createdAt.replace(/[^0-9]/g, '').slice(0, 14)}`;
  ...
  const sourceContainer = JSON.parse(sourceBytes) as {
    continuation?: { stage123Receipt?: ConnectedDevelopmentStage123ReceiptV2 };
  };
  const sourceReceipt = sourceContainer.continuation?.stage123Receipt;
  if (!sourceReceipt) throw new Error('QWEN_DEV02_SOURCE_STAGE123_RECEIPT_MISSING');
  ...
  const handoff = buildConnectedStage1RequalificationHandoffV2({ task, route, sourceReceipt });
  await writeJson(path.join(runRoot, 'stage1-requalification-handoff.json'), handoff);
  const continuation = await continueConnectedDevelopmentStage123V2({
    task, route, sourceReceipt: handoff.stage1Receipt,
  });
  ...
  const material = {
    receiptVersion: 'EDITRON_OE_QWEN_DEV02_REQUALIFICATION_RESULT_V2' as const,
    ...
    handoff, continuation, stage14Receipt, stage5Decision,
    providerCallCount: call,
  ...
  const resultPath = path.join(runRoot, 'qwen-dev02-requalification-result.json');
...
main().catch((error: unknown) => {
  ...
  process.stderr.write(`QWEN_DEV02_REQUALIFICATION_FAILED:${message.replace(/[\r\n]+/g, ' ').slice(0, 1000)}\n`);
  process.exitCode = 1;
});
