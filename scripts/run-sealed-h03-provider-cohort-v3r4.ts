...
import type { SealedH03PaidAuthorizationV3R4 }
...
const scriptPath = 'scripts/run-sealed-h03-provider-cohort-v3r4.ts';
...
async function main(): Promise<void> {
  if (option('--execute-paid-cohort') !== 'YES_I_CONFIRM_18_H03_ROWS') {
    throw new Error('SEALED_H03_V3R4_EXPLICIT_EXECUTION_CONFIRMATION_REQUIRED');
  }
  ...
  const authorization = JSON.parse(
    await readFile(path.resolve(required('--authorization')), 'utf8'),
  ) as SealedH03PaidAuthorizationV3R4;
  const [executionCommitSha, runnerSourceSha256] = await Promise.all([headSha(), fileSha(scriptPath)]);
  ...
  const outputRoot = path.resolve(option('--output-root') ?? path.join(
    ...
    `sealed-h03-provider-v3r4-run-${stamp(startedAt)}`,
  ));
  ...
  const receipt = await runSealedH03ProviderCohortV3R4({
    ...input, authorization, environment: process.env, mediaManifest, outputRoot,
    executionCommitSha, runnerSourceSha256, sandboxEnvironment, repoRoot,
    onProgress: (event) => process.stdout.write(`${JSON.stringify({
      mode: 'H03_V3R4_ROW_PROGRESS', ...event,
    })}\n`),
  });
  process.stdout.write(`${JSON.stringify({ mode: 'H03_V3R4_PAID_COHORT_COMPLETE',
    outputRoot, receiptSha256: receipt.receiptSha256, rowCount: receipt.rowCount,
    dispositionCounts: receipt.dispositionCounts, accounting: receipt.accounting,
    projectReads: receipt.projectReads, projectMutations: receipt.projectMutations }, null, 2)}\n`);
}
...
async function assertExecutionFilesCommitted(): Promise<void> {
  ...
  if (stdout.trim()) throw new Error('SEALED_H03_V3R4_EXECUTION_FILES_MUST_BE_COMMITTED');
}
...
function required(name: string): string {
  ...
  if (!value) throw new Error(`SEALED_H03_V3R4_OPTION_REQUIRED:${name}`);
  return value;
}
function confirm(name: string, expected: string): void {
  if (option(name) !== expected) throw new Error(`SEALED_H03_V3R4_CONFIRMATION_MISMATCH:${name}`);
}
