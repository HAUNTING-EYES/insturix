...
import {
  buildSealedH03ProviderOperatorInputV3R3,
  SEALED_H03_PROVIDER_IMPLEMENTATION_PATHS_V3R3,
...
const scriptPath = 'scripts/run-sealed-h03-provider-preflight-v3r3.ts';
const executionPaths = [
  ...SEALED_H03_PROVIDER_IMPLEMENTATION_PATHS_V3R3,
  'lib/editron/research/open-ended-planner/sealed-holdout-h03-provider-cohort-v3r3.ts',
  'lib/editron/research/open-ended-planner/sealed-holdout-h03-provider-preflight-v3r3.ts',
...
async function main(): Promise<void> {
  if (process.argv.includes('--run') || process.argv.includes('--dispatch')) {
    throw new Error('SEALED_H03_V3R3_PREFLIGHT_INFERENCE_FLAG_FORBIDDEN');
  }
  ...
  const preflight = await runSealedH03ProviderNoInferencePreflightV3R3({
  ...
  const outputRoot = path.resolve(option('--output-root') ?? path.join(
    ...
    `sealed-h03-provider-v3r3-preflight-${stamp(createdAt)}`,
  ));
  ...
  process.stdout.write(`${JSON.stringify({
    mode: 'H03_V3R3_ZERO_INFERENCE_PREFLIGHT',
    outputRoot,
    manifestSha256: input.cohortManifest.manifestSha256,
    absoluteMaxSpendUsd: input.cohortManifest.absoluteMaxSpendUsd,
...
async function assertExecutionFilesCommitted(): Promise<void> {
  ...
  if (stdout.trim()) throw new Error('SEALED_H03_V3R3_PREFLIGHT_EXECUTION_FILES_MUST_BE_COMMITTED');
}
...
function required(name: string): string {
  ...
  if (!value) throw new Error(`SEALED_H03_V3R3_PREFLIGHT_OPTION_REQUIRED:${name}`);
  return value;
}

function confirm(name: string, expected: string): void {
  if (option(name) !== expected) {
    throw new Error(`SEALED_H03_V3R3_PREFLIGHT_CONFIRMATION_MISMATCH:${name}`);
  }
}
