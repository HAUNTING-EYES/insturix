...
import { issueSealedH03PaidAuthorizationV3R4 }
...
const defaultRunnerPath = 'scripts/run-sealed-h03-provider-cohort-v3r4.ts';

async function main(): Promise<void> {
  if (process.argv.includes('--run')) {
    throw new Error('SEALED_H03_V3R4_AUTHORIZATION_INFERENCE_FLAG_FORBIDDEN');
  }
  ...
  const sandboxEnvironment = { snapshotId: required('--snapshot-id'),
    snapshotCommit: required('--snapshot-commit') } as const;
  ...
  const [executionCommitSha, runnerSourceSha256] = await Promise.all([headSha(), fileSha(runnerPath)]);
  const approvedAt = new Date().toISOString();
  const expiresAt = new Date(Date.parse(approvedAt) + 23 * 60 * 60_000).toISOString();
  const authorization = issueSealedH03PaidAuthorizationV3R4({
    ...
    approval: { operatorId, approvedAt, expiresAt,
      ...
      confirmedAbsoluteMaxSpendUsd: 11.673, executionCommitSha, runnerSourceSha256,
      sandboxEnvironment },
  });
  const outputPath = path.resolve(option('--output') ?? path.join(
    repoRoot, '.calibration-temp', 'open-ended-planner-v2',
    `sealed-h03-paid-authorization-v3r4-${stamp(approvedAt)}.json`,
  ));
  await writeFile(outputPath, `${JSON.stringify(authorization, null, 2)}\n`,
    { encoding: 'utf8', flag: 'wx' });
  process.stdout.write(`${JSON.stringify({ mode: 'H03_V3R4_PAID_AUTHORIZATION_ISSUED_NO_INFERENCE',
    outputPath, authorizationSha256: authorization.authorizationSha256,
    manifestSha256: authorization.manifestSha256, executionCommitSha, runnerSourceSha256,
    ...
    approvedAt, expiresAt, inferenceCallsMade: 0, projectMutationsAuthorized: 0 }, null, 2)}\n`);
}
async function assertExecutionFilesCommitted(runnerPath: string): Promise<void> {
  const paths = [
    'lib/editron/research/open-ended-planner/sealed-holdout-h03-paid-authorization-v3r4.ts',
    'lib/editron/research/open-ended-planner/sealed-holdout-h03-provider-cohort-runner-v3r4.ts',
    'scripts/issue-sealed-h03-paid-authorization-v3r4.ts', runnerPath,
  ];
  const { stdout } = await execFileAsync('git', ['status', '--porcelain', '--', ...paths],
    { cwd: repoRoot });
  if (stdout.trim()) throw new Error('SEALED_H03_V3R4_AUTHORIZATION_FILES_MUST_BE_COMMITTED');
}
async function readJson(filePath: string): Promise<Record<string, unknown>> {
  const value = JSON.parse(await readFile(filePath, 'utf8')) as unknown;
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error(`SEALED_H03_V3R4_AUTHORIZATION_JSON_INVALID:${filePath}`);
  }
...
function required(name: string): string {
  ...
  if (!value) throw new Error(`SEALED_H03_V3R4_AUTHORIZATION_OPTION_REQUIRED:${name}`);
  return value;
}
function confirm(name: string, expected: string): void {
  if (option(name) !== expected) {
    throw new Error(`SEALED_H03_V3R4_AUTHORIZATION_CONFIRMATION_MISMATCH:${name}`);
  }
}
