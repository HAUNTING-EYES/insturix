...
function buildWritingKnowledgeBlock(data, signals) {
  ...
  for (const cat of categories) {
    ...
    const top = techs[0];
    if (!top) continue;
    lines.push(`${cat.toUpperCase()} TECHNIQUE: ${top.id}`);
    ...
    if (top.weightResponse) {
      for (const [k, v] of Object.entries(top.weightResponse)) lines.push(`  IF ${k}: ${v}`);
    }
    lines.push('');
  }
  const antiAi = (data.constraints || []).filter(c => c.section === 'Anti-AI Constraints');
  if (antiAi.length > 0) {
    ...
    lines.push('  - Vary sentence rhythm. Short punch. Then a longer sentence that builds. Then punch again.');
    for (const c of antiAi) lines.push(`  - ${c.why || c.detection || ''}`);
  }
