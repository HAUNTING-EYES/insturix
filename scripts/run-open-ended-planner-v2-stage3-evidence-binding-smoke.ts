...
async function main(): Promise<void> {
  ...
  const plan = await buildStage3EvidenceBindingSmokePreflightV2() as Record<string, unknown>;
  ...
  const receipt = await runStage3EvidenceBindingSmokeV2({
    expectedPlanHash: value(args, '--plan-hash'), maxAuthorizedSpendUsd,
    operatorId: value(args, '--operator-id'), confirmedAt: new Date().toISOString(), environment: process.env,
  });
  ...
  process.stdout.write(`${JSON.stringify({
    output, planHash: receipt.planHash, receiptHash: receipt.receiptHash,
    actualProviderCostUsd: receipt.actualProviderCostUsd,
    rows: rows.map((row) => ({ rowId: row.rowId, transportDisposition: (row.run as Record<string, unknown>)?.disposition, evidenceBindingDisposition: (row.evidenceBindingEvaluation as Record<string, unknown>)?.disposition })),
  })}\n`);
}
