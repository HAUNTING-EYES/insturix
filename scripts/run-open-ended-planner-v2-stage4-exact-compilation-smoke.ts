...
async function main(): Promise<void> {
  ...
  const plan = await buildStage4ExactCompilationSmokePreflightV2() as Record<string, unknown>;
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    rows: rows.map((row) => ({ rowId: row.rowId, transportDisposition: (row.run as Record<string, unknown>)?.disposition, compilationDisposition: (row.compilationEvaluation as Record<string, unknown>)?.disposition })),
  })}\n`);
}
