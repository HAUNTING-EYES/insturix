// Brand tokens — the ONLY thing that changes between customers. Every brick reads these; none hardcode a
// value. Crucially this is NOT just a palette: it carries SHAPE (corner roundness, border weight), TYPE
// character (weight, tracking, line-height, case), DENSITY (airy↔dense spacing), DECORATION (grid/glow) and
// MOTION personality. Two brands can share a form yet feel structurally different, not just recolored.
export type Brand = {
  name: string;
  productName: string;
  colors: {
...
// Northwind — a totally different company: airy corporate light. Lighter type, roomy, very rounded, cool
// blue, NO grid/glow (flat + soft shadow), calm motion. (Fictional, to prove structural re-skin.)
