...
import {
  ...
  type MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoCollectionV1,
  type MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoRuntimeV1,
  type MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoSessionV1,
} from '@/lib/editron/services/media-source-pts-cadence-version-evidence-backfill-recovery-mongo-selector-v1';
import {
...
import { parseMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoDocumentV1 }
  from '@/lib/editron/services/media-source-pts-cadence-version-evidence-backfill-recovery-sweep-mongo-document-v1';

const CONTROLLER_ID = 'pts-cadence-version-evidence-backfill-recovery-global-v1';
...
describe('MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoSelectorV1', () => {
  ...
  it('rejects a tampered attempt policy before touching Mongo', async () => {
    ...
    await expect(selector.selectNext({
      ...
      'MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_ATTEMPT_POLICY_HASH_INVALID',
    );
  ...
  it('rejects a corrupt projected run before persisting selection state', async () => {
    ...
    await expect(selector.selectNext({
      ...
      'MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_MONGO_CANDIDATE_DOCUMENT_ENVELOPE_INVALID',
    );
  ...
  it('fails closed when the transaction callback was not committed', async () => {
    ...
    await expect(selector.selectNext({
      ...
      'MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_MONGO_TRANSACTION_NOT_COMMITTED',
    );
    expect(memory.controllers.size()).toBe(0);
    expect(memory.sweeps.size()).toBe(0);
...
function selectorFor(memory: ReturnType<typeof mongoMemory>) {
  return createMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoSelectorV1({
...
function runDocument(record: MediaSourcePtsCadenceVersionEvidenceBackfillRunRecordV1) {
  return {
    _id: record.migrationRunId,
    schemaVersion: 1,
    kind: 'EDITRON_MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RUN_DOCUMENT_V1',
...
function mongoMemory(
  ...
  const runtime: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoRuntimeV1 = {
    runs,
    controllers,
    sweeps,
...
function memorySession(
  ...
  const session: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoSessionV1 & {
    ...
    withTransaction: async <T>(
      operation: () => Promise<T>,
      options: Parameters<
        MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoSessionV1['withTransaction']
      >[1],
    ) => {
...
function memoryCollection() {
  ...
  const collection: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoCollectionV1 = {
    createIndex,
    findOne,
    findMany,
    updateOne,
