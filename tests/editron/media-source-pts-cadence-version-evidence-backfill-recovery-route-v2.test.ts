...
vi.mock(
  '@/lib/editron/services/media-source-pts-cadence-version-evidence-backfill-recovery-owner-v2',
  () => ({ recoverMediaSourcePtsCadenceVersionEvidenceBackfillSweepsV2: mocks.recovery }),
);

import { GET }
  from '@/app/api/cron/recover-media-source-pts-cadence-version-evidence-backfill/route';

const repoRoot = resolve(__dirname, '../..');
const RECEIPT_HASH = 'c'.repeat(64);

describe('media source pts cadence version evidence backfill durable recovery cron V2', () => {
  beforeEach(() => {
  ...
  it('returns a retryable receipt and sanitized alert for partial delivery', async () => {
    ...
    expect(errorLog).toHaveBeenCalledWith(
      '[MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryV2]',
      {
  ...
  it('sanitizes a recovery outage and remains retryable', async () => {
    ...
    expect(body).toEqual({
      success: false,
      error: {
        code: 'MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_UNAVAILABLE',
      },
    });
    expect(JSON.stringify(body)).not.toContain('database-secret-detail');
    expect(errorLog).toHaveBeenCalledWith(
      '[MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryV2] sweep unavailable:',
      'Error',
    );
  });
  ...
  it('is present in the deployed cron schedule', () => {
    ...
    expect(configuration.crons).toContainEqual({
      path: '/api/cron/recover-media-source-pts-cadence-version-evidence-backfill',
      schedule: '*/5 * * * *',
    });
  });
});
...
function request(secret?: string): Request {
  return new Request(
    'https://editron.example.test/api/cron/'
      + 'recover-media-source-pts-cadence-version-evidence-backfill',
    {
