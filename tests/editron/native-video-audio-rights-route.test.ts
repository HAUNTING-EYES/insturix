...
vi.mock('@/lib/editron/services/native-video-audio-rights-attestation', () => {
  ...
  return {
    reattestNativeVideoAudioRights: mocks.attest,
    NativeVideoAudioRightsAttestationError: MockAttestationError,
  };
});

import { POST } from '@/app/api/services/editron/projects/[projectId]/native-video-audio-rights/route';
import { NativeVideoAudioRightsAttestationError } from '@/lib/editron/services/native-video-audio-rights-attestation';
...
function request(body: unknown): Request {
  return new Request(
    'https://app.example.com/api/services/editron/projects/proj_legacy/native-video-audio-rights',
    {
      method: 'POST',
...
describe('native-video audio rights attestation route', () => {
  ...
  it('binds user and project identity on the server', async () => {
    const attestation = {
      accepted: true,
  ...
  it('rejects malformed and oversized JSON before the domain service', async () => {
    const malformed = new Request('https://app.example.com/native-video-audio-rights', {
      method: 'POST',
      body: '{',
    });
  ...
  it('preserves typed domain status and redacts unexpected failures', async () => {
    mocks.attest.mockRejectedValueOnce(
      new NativeVideoAudioRightsAttestationError(
    ...
    expect(await internal.json()).toEqual({
      success: false,
      error: 'Source-media rights confirmation failed',
      code: 'INTERNAL_ERROR',
    });
  });
});
