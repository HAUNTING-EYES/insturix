...
vi.mock('@/lib/editron/services/media-source-audio-recovery-runtime-v1', () => ({
  runMediaSourceAudioRecoveryV1: mocks.recovery,
}));

import { GET } from '@/app/api/cron/recover-media-source-audio/route';

const repoRoot = resolve(__dirname, '../..');

describe('media source audio recovery cron V1', () => {
  beforeEach(() => {
  ...
  it('returns success only when every selected delivery is confirmed', async () => {
    mocks.recovery.mockResolvedValue({
      ...
      results: [{
        state: 'dispatched',
        jobId: 'dwj_audio_recovery_1',
        messageId: 'qstash-audio-recovery-1',
      }],
    });
  ...
  it('keeps partial delivery retryable without hiding its receipt', async () => {
    mocks.recovery.mockResolvedValue({
      scanned: 1,
      eligible: 1,
      skipped: 0,
      results: [{
        ...
        jobId: 'dwj_audio_recovery_2',
        messageId: 'qstash-audio-recovery-2',
    ...
    await expect(response.json()).resolves.toMatchObject({
      success: false,
      recovery: {
        results: [{ jobId: 'dwj_audio_recovery_2' }],
      },
    });
  });
  ...
  it('is present in the deployed cron schedule', () => {
    ...
    expect(configuration.crons).toContainEqual({
      path: '/api/cron/recover-media-source-audio',
      schedule: '*/5 * * * *',
    });
  });
});
...
function request(secret?: string): Request {
  return new Request(
    'https://editron.example.test/api/cron/recover-media-source-audio',
    {
