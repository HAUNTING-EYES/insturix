...
describe('MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryAttemptPolicyV1', () => {
  it('creates a self-hashed immutable policy and revalidates it', () => {
    ...
    expect(assertMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryAttemptPolicyV1(
  ...
  it('rejects field expansion and any hash-bound material tamper', () => {
    ...
    expect(() => assertMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryAttemptPolicyV1({
      ...
      'MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_ATTEMPT_POLICY_FIELDS_INVALID',
    );
    expect(() => assertMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryAttemptPolicyV1({
      ...policy,
      maxAttempts: 5,
  ...
  it.each([
    ...
    expect(() => createMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryAttemptPolicyV1({
      maxAttempts: 4,
      leaseMs: 60_000,
      retryBaseMs: 1_000,
      retryMaxMs: 2_500,
  ...
  it('resolves the exact lease boundary without rounding', () => {
    expect(resolveMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryLeaseExpiryV1(
      attemptPolicy(),
  ...
  it('uses bounded exponential retry and returns null at exhaustion', () => {
    ...
    expect(resolveMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryRetryAtV1(
      policy,
      1,
      attemptedAt,
