...
describe("Editron project save payload compaction", () => {
  ...
  it("captures a writer-issued receipt from a successful overlay-family replacement", async () => {
    ...
    const captured = await projectService.captureMutationReceipts(async () => (
      projectService.replaceOverlayFamilyAtomic("user_1", "proj_1", {
  ...
  it("returns the existing stale result without a write or receipt for an overlay-family replacement", async () => {
    const currentUpdatedAt = "2026-08-11T02:31:00.000Z";
    persistenceMocks.findOne.mockResolvedValueOnce({
      projectId: "proj_1",
      userId: "user_1",
    ...
    const captured = await projectService.captureMutationReceipts(async () => (
      projectService.replaceOverlayFamilyAtomic("user_1", "proj_1", {
        expectedUpdatedAt: new Date("2026-08-11T02:30:00.000Z"),
        overlays: [],
      })
    ));

    expect(captured).toEqual({ value: false, receipts: [] });
    expect(persistenceMocks.updateOne).not.toHaveBeenCalled();
  });
