...
import {
  ...
  type MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreCollectionV1,
  type MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreRuntimeV1,
  type MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreSessionV1,
} from '@/lib/editron/services/media-source-pts-cadence-version-evidence-backfill-recovery-sweep-mongo-store-v1';
import {
...
const POLICY = createMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryAttemptPolicyV1({
  maxAttempts: 3,
  leaseMs: 1_000,
  retryBaseMs: 2_000,
  retryMaxMs: 4_000,
...
describe('MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreV1', () => {
  it('claims the earliest eligibility across due and expired-lease indexes', async () => {
    ...
    const expired = claimMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateV1(
      expiredPending,
    ...
    expect(fixture.runtime.sweeps.indexes).toEqual([
      {
        ...
        options: { name: 'pts_cadence_version_evidence_backfill_recovery_sweep_due_v1' },
      },
      {
        keys: {
          status: 1,
        ...
        options: {
          name: 'pts_cadence_version_evidence_backfill_recovery_sweep_expired_lease_v1',
        },
      },
    ]);
    expect(fixture.runtime.attempts.indexes).toEqual([{
      ...
      options: {
        name: 'uniq_pts_cadence_version_evidence_backfill_recovery_attempt_number_v1',
        unique: true,
      },
    }]);
    expect(fixture.runtime.sweeps.findCalls.slice(0, 2).map(
      ...
      'pts_cadence_version_evidence_backfill_recovery_sweep_due_v1',
      'pts_cadence_version_evidence_backfill_recovery_sweep_expired_lease_v1',
    ]);
  ...
  it('fences a stale worker after lease reclaim', async () => {
    ...
    const oldTransition =
      claimMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateV1(initial, {
...
function requiredClaim(
  value: MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepClaimV1 | null,
...
function storeFixture(
  ...states: readonly MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateV1[]
) {
  ...
  return {
    runtime,
    store: createMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreV1({
...
class MemoryRuntime implements
  ...
  > {
    ...
    const session: MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreSessionV1 = {
