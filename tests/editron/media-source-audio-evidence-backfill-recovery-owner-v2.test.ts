...
import {
  ...
  recoverMediaSourceAudioEvidenceBackfillSweepsV2,
  resolveMediaSourceAudioEvidenceBackfillRecoveryConfigurationV2,
  type MediaSourceAudioEvidenceBackfillRecoveryEnvironmentV2,
...
describe('MediaSourceAudioEvidenceBackfillRecoveryOwnerV2', () => {
  it('requires a complete bounded policy before any mutation owner runs', async () => {
    expect(() => resolveMediaSourceAudioEvidenceBackfillRecoveryConfigurationV2({
      ...environment(),
      EDITRON_MEDIA_AUDIO_EVIDENCE_BACKFILL_RECOVERY_MAX_ATTEMPTS: undefined,
    })).toThrow('OWNER_V2_MAX_ATTEMPTS_CONFIG_INVALID');
    expect(() => resolveMediaSourceAudioEvidenceBackfillRecoveryConfigurationV2({
      ...environment(),
      EDITRON_MEDIA_AUDIO_EVIDENCE_BACKFILL_RECOVERY_LEASE_MS: '59999',
    })).toThrow('OWNER_V2_LEASE_MS_INVALID');
    expect(() => resolveMediaSourceAudioEvidenceBackfillRecoveryConfigurationV2({
      ...environment(),
      EDITRON_MEDIA_AUDIO_EVIDENCE_BACKFILL_RECOVERY_RETRY_MAX_MS: '299999',
    ...
    await expect(recoverMediaSourceAudioEvidenceBackfillSweepsV2({
      environment: {
        ...environment(),
  ...
  it('binds fair selection through claim, dispatch, attempt and settlement', async () => {
    ...
    const receipt = await recoverMediaSourceAudioEvidenceBackfillSweepsV2({
      environment: environment(),
      selector,
      sweepStore,
      dispatch,
...
function selectedFixture(migrationRunIds: readonly string[]) {
  ...
  const pending = createMediaSourceAudioEvidenceBackfillRecoverySweepStateV1(
    selected.intent,
...
function settlementStore(fixture: ReturnType<typeof selectedFixture>) {
  return {
    ...
    settle: vi.fn(async (input: Readonly<{
      ...
      return {
        disposition: 'SETTLED' as const,
        state: settleMediaSourceAudioEvidenceBackfillRecoverySweepStateV1(
...
function environment(): MediaSourceAudioEvidenceBackfillRecoveryEnvironmentV2 {
  return {
    ...
    EDITRON_MEDIA_AUDIO_EVIDENCE_BACKFILL_RECOVERY_MAX_ATTEMPTS: '3',
    EDITRON_MEDIA_AUDIO_EVIDENCE_BACKFILL_RECOVERY_LEASE_MS: '120000',
    EDITRON_MEDIA_AUDIO_EVIDENCE_BACKFILL_RECOVERY_RETRY_BASE_MS: '300000',
    EDITRON_MEDIA_AUDIO_EVIDENCE_BACKFILL_RECOVERY_RETRY_MAX_MS: '600000',
  };
}
