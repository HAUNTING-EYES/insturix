...
describe('open-ended planner V2 paid-smoke preflight', () => {
  ...
  it('uses the same difficult-reference packet and condition for every comparison row', async () => {
    ...
    const comparisonRows = plan.smokeRows.filter(({ comparisonPurpose }) => comparisonPurpose === 'FAIR_ORDERED_IMAGE_SEQUENCE_COMPARISON');
    ...
    expect(new Set(comparisonRows.map(({ inputArm }) => inputArm))).toEqual(new Set(['REFERENCE_IMAGE_SEQUENCE_EVIDENCE']));
    expect(plan.smokeRows.filter(({ comparisonPurpose }) => comparisonPurpose === 'NATIVE_REFERENCE_VIDEO_COMPARISON').map(({ routeId }) => routeId).sort())
  ...
  it('records applicability for every provider, development task, and modality arm', async () => {
    const plan = await buildDevelopmentSmokePreflightV2() as Plan;
    expect(plan.routeApplicability).toHaveLength(50);
    ...
    expect(new Set(plan.routeApplicability.map(({ inputArm }) => inputArm))).toEqual(new Set([
      'MULTIMODAL', 'REFERENCE_IMAGE_SEQUENCE_EVIDENCE', 'REFERENCE_NATIVE_VIDEO_EVIDENCE', 'TEXT_EVIDENCE_ONLY',
    ]));
    ...
    expect(multimodal.every(({ modalityStatus }) => modalityStatus === 'NOT_APPLICABLE')).toBe(true);
    const nativeVideo = plan.routeApplicability.filter(({ inputArm }) => inputArm === 'REFERENCE_NATIVE_VIDEO_EVIDENCE');
    expect(nativeVideo.filter(({ routeId }) => routeId.startsWith('GOOGLE_'))
      .every(({ modalityStatus }) => modalityStatus === 'APPLICABLE')).toBe(true);
    expect(nativeVideo.filter(({ routeId }) => !routeId.startsWith('GOOGLE_'))
      .every(({ modalityStatus }) => modalityStatus === 'NOT_APPLICABLE')).toBe(true);
  });
  ...
  it('blocks the unsupported 0731 identity instead of relabelling the provider alias', async () => {
    ...
    expect(plan.excludedRows[0]).toMatchObject({
      routeId: 'DEEPSEEK_FLASH',
      comparisonPurpose: 'TEXT_ONLY_CAPABILITY_GAP_CONTROL',
      dispatchStatus: 'BLOCKED_MODEL_IDENTITY',
    });
...
type Plan = Awaited<ReturnType<typeof buildDevelopmentSmokePreflightV2>> & {
  ...
  excludedRows: Array<{ routeId: string; comparisonPurpose: string; dispatchStatus: string; blockers: string[] }>;
