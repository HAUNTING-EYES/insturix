...
SCAN_SUBMISSION_KIND_V1 = "EDITRON_MEDIA_SOURCE_PTS_SCAN_SUBMISSION_V1"
EPOCH_SCAN_SUBMISSION_KIND_V3 = "EDITRON_MEDIA_SOURCE_PTS_EPOCH_SCAN_SUBMISSION_V3"
...
async def submit_source_pts_scan(request: fastapi.Request):
    return await _submit_source_pts_scan(
        request,
...
@app.function(image=image, secrets=[private_storage_secret], timeout=120)
@modal.fastapi_endpoint(method="POST", requires_proxy_auth=True)
async def submit_source_pts_epoch_scan_v3(request: fastapi.Request):
    return await _submit_source_pts_scan(
        request,
...
async def _submit_source_pts_scan(
    request: fastapi.Request,
    submission_kind: str,
    mapper_version: str,
    command_policy_version: str,
    map_function: Any,
    include_mapper_identity: bool,
):
    ...
    call = await map_function.spawn.aio(validated)
    response = {"ok": True, "submissionId": submission_id,
                "mapBindingSha256": validated["mapBindingSha256"],
                "functionCallId": call.object_id}
    return {**response, **_transport_identity(
        mapper_version if include_mapper_identity else None,
        command_policy_version if include_mapper_identity else None,
    )}


def _validate_submission_body(
    body: object,
    submission_kind: str,
    mapper_version: str,
    command_policy_version: str,
    ...
    if not isinstance(body, dict) or sorted(body) != [
        "kind", "request", "schemaVersion", "submissionId",
            ...
            or body.get("kind") != submission_kind \
    ...
    if not is_allowed_media_source_url(
        ...
        raise ScanInputError("SCAN_SOURCE_URL_NOT_ALLOWED")
    return body["submissionId"], validated


@app.function(image=image, timeout=120)
@modal.fastapi_endpoint(method="POST", requires_proxy_auth=True)
async def poll_source_pts_scan(request: fastapi.Request):
    return await _poll_source_pts_scan(request, None, None)


@app.function(image=image, timeout=120)
@modal.fastapi_endpoint(method="POST", requires_proxy_auth=True)
async def poll_source_pts_epoch_scan_v3(request: fastapi.Request):
    return await _poll_source_pts_scan(
        request,
        EPOCH_MAPPER_VERSION_V3,
        EPOCH_COMMAND_POLICY_VERSION_V3,
    )
...
async def _poll_source_pts_scan(
    request: fastapi.Request,
    mapper_version: str | None,
    command_policy_version: str | None,
):
    from fastapi.responses import JSONResponse
    ...
    try:
        call_id, binding, submission_id = _validate_poll_body(
            body,
            mapper_version,
            command_policy_version,
        )
        ...
        return JSONResponse(status_code=400, content={"ok": False, "error": "SCAN_POLL_INVALID"})
    identity = _transport_identity(mapper_version, command_policy_version)
    try:
    ...
    return {"ok": True, "status": "TERMINAL", "submissionId": submission_id,
            "mapBindingSha256": binding, "result": result, **identity}


def _validate_poll_body(
    body: object,
    mapper_version: str | None,
    command_policy_version: str | None,
) -> tuple[str, str, str]:
    identity = _transport_identity(mapper_version, command_policy_version)
    expected = ["functionCallId", "mapBindingSha256", "submissionId", *identity]
    if not isinstance(body, dict) or sorted(body) != sorted(expected):
        raise ScanInputError("SCAN_POLL_INVALID")
    if any(body.get(field) != value for field, value in identity.items()):
        raise ScanInputError("SCAN_POLL_INVALID")
    ...
    if not isinstance(submission_id, str) or not SAFE_SUBMISSION_ID.fullmatch(submission_id):
        raise ScanInputError("SCAN_POLL_INVALID")
    return call_id, binding, submission_id
...
def _transport_identity(
    ...
    if mapper_version is None and command_policy_version is None:
        return {}
    if not mapper_version or not command_policy_version:
        raise ScanInputError("SCAN_TRANSPORT_IDENTITY_INVALID")
    return {"mapperVersion": mapper_version, "commandPolicyVersion": command_policy_version}
