...
const GhostMarker: React.FC<GhostMarkerProps> = ({
  ...
  return (
    <div
      className="absolute top-0 w-[2px] bg-zinc-500/50 dark:bg-zinc-500/50 pointer-events-none z-40"
      style={{
        left: `calc(${position}% - 1px)`,
        height: "calc(100% + 0px)",
      }}
    >
      <div className="w-0 h-0 border-l-[5px] border-r-[5px] border-t-[8px] border-l-transparent border-r-transparent border-t-zinc-500 dark:border-t-zinc-500 absolute top-[0px] left-1/2 transform -translate-x-1/2 pointer-events-none" />
    </div>
  );
};
