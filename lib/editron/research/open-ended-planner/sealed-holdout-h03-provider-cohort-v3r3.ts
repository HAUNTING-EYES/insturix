import { CAP2_CURRENT_TRUTH_REISSUE_AUDIT_V5 }
  from '../capability-census/cap2-current-truth-reissue-audit-v5';
...
import { SEALED_H03_MODEL_SOURCE_STAGE_BUDGET_V3R }
  from './sealed-holdout-h03-model-candidate-v3r';

type JsonRecord = Record<string, unknown>;

export const SEALED_H03_PROVIDER_COHORT_VERSION_V3R3 =
  'EDITRON_OE_SEALED_H03_PROVIDER_COHORT_V3R3_1' as const;
export const SEALED_H03_PROVIDER_COHORT_PATH_V3R3 =
  'lib/editron/research/open-ended-planner/sealed-holdout-h03-provider-cohort-v3r3.ts' as const;
...
const IMPLEMENTATION_BINDINGS_SHA256 =
  'fc13c4234a2af681222c4a47d1703357943d6eab1a7dc60d695523dfaf0f8cef';
...
export const SEALED_H03_PROVIDER_SOURCE_REQUEST_IDENTITY_V3R3 = deepFreezeV1({
...
export const SEALED_H03_PROVIDER_BUDGET_ARMS_V3R3 = deepFreezeV1([
  {
    armId: 'PRODUCTION_BUDGET',
    maximumSourceCandidates: 1,
    maximumVerifierRepairs: 0,
    maximumTransportAttemptsPerCandidate: 2,
    maximumProviderHttpRequests: 2,
  },
  {
    armId: 'CAPABILITY_CEILING',
    maximumSourceCandidates: 2,
    maximumVerifierRepairs: 1,
    maximumTransportAttemptsPerCandidate: 2,
    maximumProviderHttpRequests: 4,
  },
] as const);

type BudgetArm = typeof SEALED_H03_PROVIDER_BUDGET_ARMS_V3R3[number];
type ImplementationBinding = Readonly<{ path: string; sha256: string }>;

export interface SealedH03ProviderCohortManifestV3R3 {
  version: typeof SEALED_H03_PROVIDER_COHORT_VERSION_V3R3;
  authority: 'RESEARCH_ONLY_NO_PROJECT_MUTATION_NO_PROVIDER_DISPATCH';
  contractSource: Readonly<{ path: typeof SEALED_H03_PROVIDER_COHORT_PATH_V3R3; sha256: string }>;
  ...
  sourceRequestIdentity: typeof SEALED_H03_PROVIDER_SOURCE_REQUEST_IDENTITY_V3R3;
  ...
  budgetArms: typeof SEALED_H03_PROVIDER_BUDGET_ARMS_V3R3;
...
export function buildSealedH03ProviderCohortManifestV3R3(input: Readonly<{
  ...
  implementationBindings: readonly ImplementationBinding[];
}>): Readonly<SealedH03ProviderCohortManifestV3R3> {
  requireSha(input.contractSourceSha256, 'SEALED_H03_PROVIDER_COHORT_SOURCE_HASH_INVALID');
  ...
  if (base.manifestSha256 !== BASE_MANIFEST_SHA256
    ...
    fail('SEALED_H03_PROVIDER_COHORT_BASE_DRIFT');
  }
  const h03 = base.cases.find(({ caseId }) => caseId === 'HOLD-03:C1');
  if (!h03 || h03.publicCaseSha256 !== H03_PUBLIC_CASE_SHA256) {
    fail('SEALED_H03_PROVIDER_COHORT_CASE_DRIFT');
  }
  const implementationBindings = validateImplementationBindings(input.implementationBindings);
  const providerRoutes = provider.routes.map((entry) => deepFreezeV1({
    route: entry.route,
    transport: entry.transport,
    pricing: entry.pricing,
    priceSnapshotDate: entry.priceSnapshotDate,
    pricingSource: entry.pricingSource,
  }));
  const rows = buildRows(providerRoutes, SEALED_H03_PROVIDER_BUDGET_ARMS_V3R3);
  const material = {
    version: SEALED_H03_PROVIDER_COHORT_VERSION_V3R3,
    authority: 'RESEARCH_ONLY_NO_PROJECT_MUTATION_NO_PROVIDER_DISPATCH' as const,
    contractSource: {
      path: SEALED_H03_PROVIDER_COHORT_PATH_V3R3,
    ...
    cap2CurrentTruthBinding: {
      ...
      manifestSha256: CAP2_CURRENT_TRUTH_REISSUE_AUDIT_V5.manifestHash,
      normalizedSourceSnapshotSha256:
        CAP2_CURRENT_TRUTH_REISSUE_AUDIT_V5.sourceBinding.normalizedSourceSnapshotHash,
      runtimeAuthorityDenied: true as const,
    },
    // The provider manifest owns route/pricing identity only; CAP authority is V5 above.
    providerRouteManifest: {
      purpose: 'ROUTE_PRICING_AND_PROVIDER_IDENTITY_ONLY_NOT_CAP_AUTHORITY',
    ...
    h03CaseBinding: {
      caseId: h03.caseId,
      publicCaseSha256: h03.publicCaseSha256,
      ownerOnlySha256: h03.ownerOnlySha256,
      evaluatorOnlySha256: h03.evaluatorOnlySha256,
    },
    sourceRequestIdentity: SEALED_H03_PROVIDER_SOURCE_REQUEST_IDENTITY_V3R3,
    implementationBindings,
    implementationBindingsSha256: IMPLEMENTATION_BINDINGS_SHA256,
    stageBudget: SEALED_H03_MODEL_SOURCE_STAGE_BUDGET_V3R,
    budgetArms: SEALED_H03_PROVIDER_BUDGET_ARMS_V3R3,
...
export function assertSealedH03ProviderCohortManifestV3R3(
  value: unknown,
): Readonly<SealedH03ProviderCohortManifestV3R3> {
  if (!isRecord(value)) fail('SEALED_H03_PROVIDER_COHORT_MISSING');
  const candidate = value as unknown as SealedH03ProviderCohortManifestV3R3;
  ...
  const baseSource = record(base.contractSource);
  ...
  const rows = buildRows(routes, SEALED_H03_PROVIDER_BUDGET_ARMS_V3R3);
  if (candidate.version !== SEALED_H03_PROVIDER_COHORT_VERSION_V3R3
    || candidate.authority !== 'RESEARCH_ONLY_NO_PROJECT_MUTATION_NO_PROVIDER_DISPATCH'
    || candidate.contractSource.path !== SEALED_H03_PROVIDER_COHORT_PATH_V3R3
    || base.manifestSha256 !== BASE_MANIFEST_SHA256
    || baseSource.sha256 !== BASE_CONTRACT_SHA256
    || cap2.artifactType !== CAP2_CURRENT_TRUTH_REISSUE_AUDIT_V5.artifactType
    || cap2.manifestSha256 !== CAP2_CURRENT_TRUTH_REISSUE_AUDIT_V5.manifestHash
    ...
    || hashCanonicalJsonV1(candidate.sourceRequestIdentity)
      !== hashCanonicalJsonV1(SEALED_H03_PROVIDER_SOURCE_REQUEST_IDENTITY_V3R3)
    || hashCanonicalJsonV1(candidate.budgetArms)
      !== hashCanonicalJsonV1(SEALED_H03_PROVIDER_BUDGET_ARMS_V3R3)
    || candidate.implementationBindingsSha256 !== IMPLEMENTATION_BINDINGS_SHA256
    || candidate.repetitionsPerRouteArm !== 3
    || candidate.rows.length !== 18
    ...
    fail('SEALED_H03_PROVIDER_COHORT_DRIFT');
  }
...
function buildRows(
  ...
  return routes.flatMap((entry) => arms.flatMap((arm) => [1, 2, 3].map((repetition) => ({
    ...
    absoluteMaxRowSpendUsd: roundUsd(
      maximumProviderRequestSpend(entry) * arm.maximumProviderHttpRequests,
    ),
  }))));
}
...
function maximumProviderRequestSpend(entry: Readonly<ProviderNativeCohortRouteV2R>): number {
  ...
  const spend = (
    SEALED_H03_MODEL_SOURCE_STAGE_BUDGET_V3R.maxInputTokens * inputRate
    + outputTokens * entry.pricing.outputUsdPerMillion
  ) / 1_000_000;
  if (spend > SEALED_H03_MODEL_SOURCE_STAGE_BUDGET_V3R.maxProviderCostUsd) {
    fail(`SEALED_H03_PROVIDER_ROUTE_EXCEEDS_STAGE_COST:${entry.route.routeId}`);
  }
  return spend;
}

function validateImplementationBindings(
  values: readonly ImplementationBinding[],
): readonly ImplementationBinding[] {
  const sorted = [...values].sort((left, right) => left.path < right.path ? -1 : 1);
  if (sorted.length !== IMPLEMENTATION_PATHS.length
    || sorted.some((entry, index) => entry.path !== IMPLEMENTATION_PATHS[index]
      || !/^[a-f0-9]{64}$/.test(entry.sha256))
    || hashCanonicalJsonV1(sorted) !== IMPLEMENTATION_BINDINGS_SHA256) {
    fail('SEALED_H03_PROVIDER_IMPLEMENTATION_BINDING_DRIFT');
  }
  return deepFreezeV1(sorted);
}
function sameRouteSet(routes: readonly Readonly<ProviderNativeCohortRouteV2R>[]): boolean {
  return hashCanonicalJsonV1(routes.map(({ route }) => [
    route.routeId, route.provider, route.model,
  ])) === hashCanonicalJsonV1([
