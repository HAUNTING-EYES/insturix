...
function evidenceBoundIntentV2R(conditionId: Dev03ConditionV2): JsonRecord {
  ...
  const allBindings = baseline
    ? ['bind-project', 'bind-timeline', 'bind-beats', 'bind-beat-sync-constraints', 'bind-protected-audio']
    : ['bind-project', 'bind-timeline', 'bind-beat-sync-constraints', 'bind-protected-audio'];
  ...
  return {
    ...
    nodes: [
      ...
      node('node-align-boundaries', 'sync_cuts_to_beats', ['bind-beats', 'bind-timeline', 'bind-beat-sync-constraints', 'bind-protected-audio'], ['proof-boundary-timing', 'proof-source-handles', 'proof-protected-audio'], beatStatus, unresolved),
    ...
    evidenceBindings: [
      ...
      { bindingId: 'bind-beat-sync-constraints', factIds: ['fact-beat-sync-constraints'], nodeIds: ['node-align-boundaries', 'node-proof-project', 'node-proof-timeline'], status: 'BOUND' },
...
function evidencePack(conditionId: Dev03ConditionV2, receipt: Readonly<Dev03MeasuredEvidenceReceiptV2>, receiptHash: string): JsonRecord {
  ...
  const facts: JsonRecord[] = [
    ...
    { factId: 'fact-beat-sync-constraints', kind: 'BEAT_SYNC_CONSTRAINTS', evidenceIds: ['EV-DEV03-D1', 'EV-DEV03-T1'], constraints: { maxSnapFrames: fixture.evidence.maxBoundaryShiftFrames, minClipFrames: 30, maxConsecutiveBeatCuts: 4, protectedAudioRange: { startFrame: fixture.evidence.protectedAudioRange[0], endFrame: fixture.evidence.protectedAudioRange[1] }, protectedBoundaryToleranceFrames: 2, sourceDurationFramesByAssetId: { [fixture.assets.cards.assetId]: fixture.assets.cards.durationInFrames }, requireSourceHandles: true } },
