...
import { buildProviderNativeGeneratedSourceRouteV2R }
...
import {
  ...
  SEALED_H03_PROVIDER_SOURCE_REQUEST_IDENTITY_V3R4,
...
type FetchV3R4 = typeof fetch;

export const SEALED_H03_PROVIDER_PREFLIGHT_VERSION_V3R4 =
  'EDITRON_OE_SEALED_H03_PROVIDER_PREFLIGHT_V3R4_1' as const;

export interface SealedH03ProviderSourceRequestV3R4 {
...
export async function runSealedH03ProviderNoInferencePreflightV3R4(input: {
  ...
  assertProviderInfrastructure(provider, input.providerInfrastructureReceipt,
  ...
  if (routeManifest.manifestSha256 !== provider.manifestSha256) {
    fail('SEALED_H03_V3R4_PREFLIGHT_ROUTE_MANIFEST_DRIFT');
  }
  ...
  for (const routeEntry of provider.routes) {
    ...
    if (direct.provider !== 'openai' && direct.provider !== 'google') {
      fail(`SEALED_H03_V3R4_PREFLIGHT_PROVIDER_UNSUPPORTED:${direct.provider}`);
    }
    ...
    if (codecRoute.kind === 'google') {
      ...
      const response = await fetchImpl(countRequest.endpoint, {
        method: 'POST', headers: countRequest.headers,
        body: JSON.stringify(countRequest.body),
      });
      if (!response.ok) fail(`SEALED_H03_V3R4_PREFLIGHT_COUNT_FAILED:${response.status}`);
      const body = await response.json() as { totalTokens?: unknown };
      if (!Number.isSafeInteger(body.totalTokens) || Number(body.totalTokens) < 0) {
        fail('SEALED_H03_V3R4_PREFLIGHT_COUNT_INVALID');
      }
    ...
    if (boundedInputTokens > packet.packet.stageBudget.maxInputTokens) {
      fail(`SEALED_H03_V3R4_PREFLIGHT_INPUT_BUDGET_EXCEEDED:${direct.routeId}`);
    }
    for (const arm of manifest.budgetArms) {
      ...
      if (!row) {
        fail(`SEALED_H03_V3R4_PREFLIGHT_ROW_MISSING:${direct.routeId}:${arm.armId}`);
      }
      checks.push({
        routeId: direct.routeId, armId: arm.armId,
        ...
        tokenCountMethod, boundedInputTokens,
  ...
  if (checks.length !== 6 || h03GoogleCountTokensPosts !== 1) {
    fail('SEALED_H03_V3R4_PREFLIGHT_CHECK_SET_DRIFT');
  }
  const inheritedNetwork = input.providerInfrastructureReceipt.networkCalls;
  const material = {
    version: SEALED_H03_PROVIDER_PREFLIGHT_VERSION_V3R4,
    ...
    sourceRequest: {
      ...
      correctedProviderContractSha256:
        hashCanonicalJsonV1(SEALED_H03_PROVIDER_SOURCE_REQUEST_IDENTITY_V3R4.correctedProviderContract),
    },
    checks,
...
function assertSourceRequest(
  manifest: Readonly<SealedH03ProviderCohortManifestV3R4>,
  request: Readonly<SealedH03ProviderSourceRequestV3R4>,
): Readonly<HashedStagePacketV2> {
  const identity = SEALED_H03_PROVIDER_SOURCE_REQUEST_IDENTITY_V3R4;
  ...
  if (request.apiImplementationHash !== apiBinding?.sha256
    ...
    || manifest.sandboxWorkerImplementationSha256
      !== identity.correctedProviderContract.sandboxWorkerImplementationSha256) {
    fail('SEALED_H03_V3R4_PREFLIGHT_SOURCE_IDENTITY_DRIFT');
  }
  ...
  if (hashCanonicalJsonV1(rebuilt) !== hashCanonicalJsonV1(request.packet)) {
    fail('SEALED_H03_V3R4_PREFLIGHT_PACKET_DRIFT');
  }
  return rebuilt;
}
function assertProviderInfrastructure(
  ...
  if (receipt.manifestSha256 !== provider.manifestSha256
    ...
    fail('SEALED_H03_V3R4_PREFLIGHT_INFRASTRUCTURE_RECEIPT_INVALID');
  }
}
function requiredKey(environment: Readonly<Record<string, string | undefined>>,
  provider: 'openai' | 'google'): string {
  const value = provider === 'openai' ? environment.OPENAI_API_KEY
    : environment.GEMINI_API_KEY ?? environment.GOOGLE_API_KEY;
  if (!value?.trim()) fail(`SEALED_H03_V3R4_PREFLIGHT_KEY_MISSING:${provider}`);
  return value.trim();
}
