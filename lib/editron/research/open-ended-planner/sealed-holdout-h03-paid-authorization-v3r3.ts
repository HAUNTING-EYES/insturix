...
import {
  SEALED_H03_PROVIDER_PREFLIGHT_VERSION_V3R3,
  type runSealedH03ProviderNoInferencePreflightV3R3,
...
export function issueSealedH03PaidAuthorizationV3R3(input: Readonly<{
  ...
  if (!approval.operatorId.trim() || !Number.isFinite(approvedAtMs)
    ...
    || approval.confirmedH03PreflightReceiptSha256
      !== input.h03PreflightReceipt.receiptSha256
  ...
  if (authorizedRows.length !== 18 || maximumProviderHttpRequests !== 54
    ...
    fail('SEALED_H03_PAID_AUTHORIZATION_LIMIT_DRIFT');
  }
  const material = {
    version: SEALED_H03_PAID_AUTHORIZATION_VERSION_V3R3,
    authority: 'BOUNDED_RESEARCH_PROVIDER_AND_SANDBOX_EXECUTION_NO_PROJECT_MUTATION' as const,
    authorizationId: `h03-v3r3-${approval.approvedAt.replace(/[-:.TZ]/g, '').slice(0, 14)}`,
    operatorId: approval.operatorId.trim(),
...
export function assertSealedH03PaidAuthorizationV3R3(
  value: unknown,
  expected: Readonly<{
    manifest: Readonly<SealedH03ProviderCohortManifestV3R3>;
    executionCommitSha: string;
  ...
  if (!isRecord(value)) fail('SEALED_H03_PAID_AUTHORIZATION_MISSING');
  const candidate = value as unknown as SealedH03PaidAuthorizationV3R3;
  const { authorizationSha256, ...material } = candidate;
  const now = expected.nowUnixMs ?? Date.now();
  const manifest = assertSealedH03ProviderCohortManifestV3R3(expected.manifest);
  ...
  const approvedAtMs = Date.parse(candidate.approvedAt);
  const expiresAtMs = Date.parse(candidate.expiresAt);
  if (candidate.version !== SEALED_H03_PAID_AUTHORIZATION_VERSION_V3R3
    || candidate.authority
    ...
    || candidate.projectReadsAuthorized !== 0
    || candidate.projectMutationsAuthorized !== 0
    || candidate.stateEffects.length
    || authorizationSha256 !== hashCanonicalJsonV1(material)) {
    fail('SEALED_H03_PAID_AUTHORIZATION_DRIFT');
  }
  return deepFreezeV1(candidate);
}

function assertPreflightChain(
  input: Readonly<{
    providerInfrastructureReceipt: Readonly<ProviderNativeNoSpendPreflightReceiptV2R>;
    h03PreflightReceipt: Readonly<H03Preflight>;
    operatorPreflightReceipt: Readonly<JsonRecord>;
  ...
  if (infrastructure.infrastructureAssessment !== 'PASS'
    ...
    || h03.version !== SEALED_H03_PROVIDER_PREFLIGHT_VERSION_V3R3
    ...
    || h03.stateEffects.length
    || h03Sha !== hashCanonicalJsonV1(h03Material)
    || operator.version !== 'EDITRON_OE_SEALED_H03_PROVIDER_OPERATOR_PREFLIGHT_V3R3_1'
    ...
    || !Number.isFinite(operatorCreatedAt) || operatorCreatedAt < nowUnixMs - 2 * 60 * 60_000
    || operatorSha !== hashCanonicalJsonV1(operatorMaterial)) {
    fail('SEALED_H03_PAID_AUTHORIZATION_PREFLIGHT_CHAIN_INVALID');
  }
}
