...
import {
  ...
  type SealedH03PaidAuthorizationV3R4,
} from './sealed-holdout-h03-paid-authorization-v3r4';
import type { SealedH03ProviderCohortManifestV3R4 }
  from './sealed-holdout-h03-provider-cohort-v3r4';
import type { SealedH03ProviderSourceRequestV3R4 }
  from './sealed-holdout-h03-provider-preflight-v3r4';
import { runSealedH03ProviderRowV3R3 }
  from './sealed-holdout-h03-provider-row-runner-v3r3';
import type { SealedHoldoutCohortManifestV3R2 } from './sealed-holdout-cohort-v3r2';

type JsonRecord = Record<string, unknown>;
type CohortRow = SealedH03ProviderCohortManifestV3R4['rows'][number];
type RowResult = Awaited<ReturnType<typeof runSealedH03ProviderRowV3R3>>;
type V3R3RowInput = Parameters<typeof runSealedH03ProviderRowV3R3>[0];

interface V3R4RowInput {
  baseManifest: Readonly<SealedHoldoutCohortManifestV3R2>;
  cohortManifest: Readonly<SealedH03ProviderCohortManifestV3R4>;
  routeEntry: ProviderNativeCohortManifestV2R['routes'][number];
  row: Readonly<CohortRow>;
  sourceRequest: Readonly<SealedH03ProviderSourceRequestV3R4>;
...
type RunRowV3R4 = (input: Readonly<V3R4RowInput>) => Promise<RowResult>;

export const SEALED_H03_PROVIDER_COHORT_RUNNER_VERSION_V3R4 =
  'EDITRON_OE_SEALED_H03_PROVIDER_COHORT_RUNNER_V3R4_1' as const;

export async function runSealedH03ProviderCohortV3R4(input: Readonly<{
  baseManifest: Readonly<SealedHoldoutCohortManifestV3R2>;
  providerManifest: Readonly<ProviderNativeCohortManifestV2R>;
  cohortManifest: Readonly<SealedH03ProviderCohortManifestV3R4>;
  sourceRequest: Readonly<SealedH03ProviderSourceRequestV3R4>;
  authorization: Readonly<SealedH03PaidAuthorizationV3R4>;
  environment: Readonly<Record<string, string | undefined>>;
  mediaManifest: Readonly<HoldoutMediaManifestV2R>;
  outputRoot: string;
  ...
  // V3R4 changes the provider-visible contract and cohort identity, not the
  // sole row execution owner. Its exact V3R3 source is hash-bound by V3R4.
  const runRow = input.runRow ?? runStableRowOwner;
  ...
  for (const row of input.cohortManifest.rows) {
    ...
    if (!routeEntry) fail(`SEALED_H03_V3R4_COHORT_ROUTE_MISSING:${row.routeId}`);
    ...
    if (existing) {
      outcomes.push(existing); progress(input, row, 'RESUMED_EXISTING_ROW'); continue;
    }
    const attemptPath = path.join(rowRoot, 'attempt.json');
    if (await exists(attemptPath)) {
      fail(`SEALED_H03_V3R4_ROW_PRIOR_ATTEMPT_INDETERMINATE:${row.rowId}`);
    }
    await writeJsonOnce(attemptPath, signed({
      version: SEALED_H03_PROVIDER_COHORT_RUNNER_VERSION_V3R4,
      authority: 'ROW_ATTEMPT_MARKER_NO_PROJECT_MUTATION',
      contractSha256: contract.contractSha256, rowId: row.rowId, createdAt: now(),
    }));
    progress(input, row, 'STARTED');
    try {
      const result = await runRow({
        baseManifest: input.baseManifest, cohortManifest: input.cohortManifest,
        routeEntry, row, sourceRequest: input.sourceRequest,
        environment: input.environment, mediaManifest: input.mediaManifest,
        outputDirectory: path.join(rowRoot, 'proof'),
        executionId: `${authorization.authorizationId}-${row.rowId}`,
        createdAt: now(), sandboxEnvironment: input.sandboxEnvironment,
      ...
      outcomes.push(outcome); progress(input, row, String(result.receipt.disposition));
    } catch (error) {
      const outcome = unverifiableOutcome(input, contract, row, error, now());
      await writeJsonOnce(failurePath, outcome);
      outcomes.push(outcome); progress(input, row, 'ROW_UNVERIFIABLE');
    }
  ...
  const material = {
    ...
    rowCount: outcomes.length, dispositionCounts: countDispositions(outcomes),
...
async function runStableRowOwner(input: Readonly<V3R4RowInput>): Promise<RowResult> {
  return runSealedH03ProviderRowV3R3(input as unknown as V3R3RowInput);
}
function validateAuthorization(input: Parameters<typeof runSealedH03ProviderCohortV3R4>[0], nowUnixMs: number) {
  return assertSealedH03PaidAuthorizationV3R4(input.authorization, {
    manifest: input.cohortManifest, executionCommitSha: input.executionCommitSha,
    runnerSourceSha256: input.runnerSourceSha256,
    sandboxEnvironment: input.sandboxEnvironment, nowUnixMs,
  });
}
function buildContract(input: Parameters<typeof runSealedH03ProviderCohortV3R4>[0],
  authorization: Readonly<SealedH03PaidAuthorizationV3R4>) {
  if (!/^[a-f0-9]{40}$/.test(input.executionCommitSha)
    || !/^[a-f0-9]{64}$/.test(input.runnerSourceSha256)) {
    fail('SEALED_H03_V3R4_COHORT_EXECUTION_IDENTITY_INVALID');
  }
  const material = {
    version: SEALED_H03_PROVIDER_COHORT_RUNNER_VERSION_V3R4,
    ...
    runnerSourceSha256: input.runnerSourceSha256,
    stableRowOwnerVersion: 'EDITRON_OE_SEALED_H03_PROVIDER_ROW_V3R3_1',
    manifestSha256: input.cohortManifest.manifestSha256,
    authorizationSha256: authorization.authorizationSha256,
    ...
    projectReadsAuthorized: 0, projectMutationsAuthorized: 0, stateEffects: [],
  };
  return deepFreezeV1({ ...material, contractSha256: hashCanonicalJsonV1(material) });
}
function successfulOutcome(contract: Readonly<JsonRecord>, row: Readonly<CohortRow>,
  ...
  return signed({ authority: 'RESEARCH_PROVIDER_ROW_OUTCOME_NO_PROJECT_MUTATION',
    contractSha256: contract.contractSha256, rowId: row.rowId,
    ...
    knownActualSpendUsd: accounting.actualSpendUsd, committedSpendUsd,
    projectReads: 0, projectMutations: 0, stateEffects: [] });
}
function unverifiableOutcome(input: Parameters<typeof runSealedH03ProviderCohortV3R4>[0],
  contract: Readonly<JsonRecord>, row: Readonly<CohortRow>, error: unknown, createdAt: string) {
  return signed({ authority: 'RESEARCH_PROVIDER_ROW_OUTCOME_NO_PROJECT_MUTATION',
    contractSha256: contract.contractSha256, rowId: row.rowId,
    disposition: 'ROW_UNVERIFIABLE',
    diagnostic: redact(error instanceof Error ? error.message : String(error), input.environment),
    createdAt, providerGeneratedCandidates: row.armId === 'CAPABILITY_CEILING' ? 2 : 1,
    providerHttpAttemptsCommitted: Number(row.maximumProviderHttpRequests),
    knownActualSpendUsd: 0, committedSpendUsd: Number(row.absoluteMaxRowSpendUsd),
    projectReads: 0, projectMutations: 0, stateEffects: [] });
}
function assertTotals(outcomes: readonly Readonly<JsonRecord>[],
  authorization: Readonly<SealedH03PaidAuthorizationV3R4>) {
  const totals = deepFreezeV1({ providerGeneratedCandidates: sum(outcomes, 'providerGeneratedCandidates'),
  ...
  if (totals.providerHttpAttemptsCommitted > authorization.limits.maximumProviderHttpRequests
    ...
    fail('SEALED_H03_V3R4_COHORT_ACCOUNTING_LIMIT_EXCEEDED');
  }
  return totals;
}
async function initializeRun(root: string, contract: Readonly<JsonRecord>): Promise<void> {
  await mkdir(path.dirname(root), { recursive: true }); await mkdir(root, { recursive: true });
  const target = path.join(root, 'run-contract.json');
  if (await exists(target)) {
    if ((await readJson(target)).contractSha256 !== contract.contractSha256) {
      fail('SEALED_H03_V3R4_COHORT_RESUME_CONTRACT_DRIFT');
    }
  } else await writeJsonOnce(target, contract);
  await mkdir(path.join(root, 'rows'), { recursive: true });
}
async function readExistingOutcome(receiptPath: string, failurePath: string,
  contract: Readonly<JsonRecord>, row: Readonly<CohortRow>): Promise<JsonRecord | null> {
  const selected = await exists(receiptPath) ? receiptPath : await exists(failurePath) ? failurePath : null;
  if (!selected) return null;
  const value = await readJson(selected); const { receiptSha256, ...material } = value;
  if (value.contractSha256 !== contract.contractSha256 || value.rowId !== row.rowId
    || receiptSha256 !== hashCanonicalJsonV1(material)) {
    fail(`SEALED_H03_V3R4_COHORT_RESUME_ROW_DRIFT:${row.rowId}`);
  }
  return value;
}
...
function countDispositions(values: readonly JsonRecord[]): JsonRecord {
  return values.reduce<JsonRecord>((counts, row) => {
    const key = String(row.disposition); counts[key] = Number(counts[key] ?? 0) + 1; return counts;
  }, {});
}
function sum(values: readonly JsonRecord[], key: string): number {
  return values.reduce((total, value) => total + Number(value[key] ?? 0), 0);
}
function progress(input: Parameters<typeof runSealedH03ProviderCohortV3R4>[0],
  row: Readonly<CohortRow>, status: string): void {
...
async function readJson(filePath: string): Promise<JsonRecord> {
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value)) fail('SEALED_H03_V3R4_JSON_INVALID');
