...
import { buildProviderNativeCohortManifestV2R }
  from './provider-native-cohort-manifest-v2r';
import {
  buildSealedH03ProviderCohortManifestV3R4,
...
export async function buildSealedH03ProviderOperatorInputV3R4(
  ...
  const [implementationBindings, sandboxOverlay] = await Promise.all([
    Promise.all(SEALED_H03_PROVIDER_IMPLEMENTATION_PATHS_V3R4.map(async (filePath) => ({
      path: filePath,
      sha256: await fileSha(repoRoot, filePath),
    }))),
    resolveGeneratedCompositionSandboxOverlayV1(repoRoot),
  ]);
  const cohortManifest = buildSealedH03ProviderCohortManifestV3R4({
    ...
    sandboxWorkerImplementationSha256: sandboxOverlay.workerImplementationHash,
  });
  const apiImplementationHash = implementationBindings.find(({ path: filePath }) => (
    filePath.endsWith('/generated-composition-api-v1.tsx')
  ))?.sha256;
  if (!apiImplementationHash) throw new Error('SEALED_H03_V3R4_API_BINDING_MISSING');
  const identity = SEALED_H03_PROVIDER_SOURCE_REQUEST_IDENTITY_V3R4;
  const argumentsValue = generatedArguments();
  if (hashCanonicalJsonV1(argumentsValue) !== identity.orchestratorArgumentsSha256) {
    throw new Error('SEALED_H03_V3R4_ORCHESTRATOR_ARGUMENT_DRIFT');
  }
  const sourceRequest: Readonly<SealedH03ProviderSourceRequestV3R4> = Object.freeze({
    apiImplementationHash,
    sourceAArtifactSha256: identity.sourceAArtifactSha256,
    sourceBArtifactSha256: identity.sourceBArtifactSha256,
    arguments: argumentsValue,
    orchestratorSpecSha256: identity.orchestratorArgumentsSha256,
    ownerAuthorizationOutputSha256: identity.ownerAuthorizationOutputSha256,
