...
const STAGE_BUDGETS: Record<StageV2, StageBudgetV2> = {
  ...
  2: { maxInputTokens: 60000, maxVisibleOutputTokens: 16000, maxReasoningTokens: 5000, maxWallClockMs: 420000, maxProviderCostUsd: 0.70 },
...
const STAGE_INSTRUCTIONS: Record<StageV2, string[]> = {
  ...
  2: ['Select editorial operations and their dependencies.', 'Prove structural hard-target coverage for every candidate execution form.', 'COVERED means the claim is assigned to catalog capabilities that can structurally realize it; missing concrete project IDs, ranges, or evidence belong in unresolvedRequirements for Stage 3 and do not by themselves make structural coverage UNVERIFIABLE.', 'ELIGIBLE requires truthful current support and compiler status; an ideal architectural route may still be selected while its candidate is INELIGIBLE or UNVERIFIABLE and records a capability gap.', 'Classify a bounded generated island as GENERATED_COMPOSITION and a full native plan surrounding an island as HYBRID.', 'Obey the assigned routing experiment.', 'Do not serialize compiler-bound runtime arguments. Supply only semantic nodeInputs explicitly assigned to the model by modelInput.plannerInputOwnership.'],
  3: ['Bind every intent to supplied evidence, rights, privacy, revision, preservation, and proof requirements.', 'Re-evaluate every Stage-2 unresolved requirement against the supplied Stage-3 facts. Omit requirements that those facts resolve; do not copy stale uncertainty forward.', 'READY_FOR_COMPILATION is legal only when no unresolved requirement has failureDisposition STOP_BEFORE_COMPILATION_OR_RENDER. If a required fact remains absent, use the matching non-ready stageDisposition.', 'BOUND means the supplied facts are complete and exact for this binding; do not downgrade a binding merely because its capability cannot execute yet.', 'PLANNED means a proof obligation is required at a later executable stage; use UNVERIFIABLE only when supplied facts cannot determine whether that proof is required, not because the proof has not run yet.', 'COMPLIANT means the proposed plan obeys the rights or privacy policy even when that policy denies listed actions; use POLICY_BLOCKED only when the proposed plan itself requires a forbidden action.', 'Express unavailable execution only through stageDisposition and unresolvedRequirements so evidence completeness remains distinct from capability readiness.', 'An EVIDENCE or AMBIGUITY requirement may be UNVERIFIABLE; only a CAPABILITY requirement may be CAPABILITY_GAP.', 'Do not invent evidence or capability.'],
