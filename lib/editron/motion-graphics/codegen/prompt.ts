...
The ordered images show ONE transparent animation composited over a real footage frame. Judge the ADDED graphic only: brand fidelity, composition, visible motion development, mobile legibility, clipping, contrast, safe-region compliance, subject/caption obstruction, and faithfulness to the licensed fact.

IMAGE TRUTH:
- The FULL COMPOSITES (labelled intro, build, settled hold) are sequential phases of the same animation, each over its matching real final edited-canvas frame. They are the authority for placement, hierarchy, subject/caption collision, and whether the graphic belongs in the shot. Inspect EACH independently at useful resolution, then judge the progression. They are not simultaneous elements.
- Any image labelled DETAIL is a native-scale crop of the graphic's OWN region over its footage. Use DETAIL images ONLY for typography, legibility, clipping, and glyph quality — NEVER for placement or subject collision (they are cropped to the graphic and cannot show where it sits in the frame).
- The LAST image is a contrast-only stress sheet. Its columns are intro/build/settled-hold; its top row is dark and bottom row is light. Use it only for alpha and contrast, never for placement or subject-obstruction judgments.
...
- composition: does the graphic sit in the frame's ROOM, clear of the subject and existing text/caption, with intentional negative space — not stretched to fill every quadrant? Subject/caption collision or dead-quadrant filling = low. The LICENSED FACT JSON carries subjectOverlap geometry when known (subject: frame fractions; coveredPct: code-measured opaque coverage of that box; hardVeto: code-verified opaque cover). A subject COLLISION is an OPAQUE plate/figure actually covering the subject — subjectOverlap.hardVeto true, or an opaque plate/figure you can SEE covering a face/hands/visual point in a composite. Transparent or haloed type near/over the subject is NOT a collision — cramped proximity or poor negative space is scored here. Dead-quadrant filling also = low.
...
AUTOMATIC REJECT (forces a reject-class score, and faithful=false for fabrication): unsupported asserted content; any colour outside brand tokens; clipped/overflowing/broken text — CHECK THE FRAME EDGES SPECIFICALLY: any text or panel touching or cut off by the right/left/bottom frame edge is CLIPPED even if it superficially resembles an intentional bleed; a keyword-highlight or lower-third-template look instead of a bespoke composition; an opaque full-canvas graphic that hides the footage; or a graphic that cannot read over the real footage and both stress backgrounds. A subject COLLISION as defined in composition (an OPAQUE plate/figure actually covering the subject — see subjectOverlap.hardVeto), or an opaque panel you can SEE covering existing on-screen caption text, is also an automatic reject.
SCORING DISCIPLINE: \`score\` is a holistic 0-10 OVERALL that MUST be consistent with the dimensions and the issues — it is NOT their mean, but it can never exceed the lowest reject-class dimension. Any reject-class problem — fabrication (faithful=false), subject/caption interference, an opaque field hiding the footage, NO visible motion development across the three phases, clipped/broken text, unreadable contrast, a non-brand colour, or template-like form — forces its \`hardFailures\` field true, \`score\` to at most 4, AND the matching dimension to at most 4. Subject interference is HARD ONLY when (a) \`subjectOverlap.hardVeto\` is TRUE — the calibrated precise-geometry veto is ENABLED and code-verified an opaque cover — or (b) you can SEE an opaque plate/figure actually over a face, hands, or the shot's visual point. When \`subjectOverlap.hardVeto\` is false or missing, subject proximity or competition is a \`composition\` concern, never a hard failure. Transparent or haloed type near/over the subject is NOT subject interference. Caption interference is HARD ONLY against a provided \`captionBoxes\` rect (an opaque panel inside it); before caption rects are provided, caption crowding is \`composition\`/typography feedback, never an auto-reject. MINIMUM-VIABLE TEXT is a quality cap: if \`form\` ≤ 4 (undesigned bare-text output), \`score\` is at most 6 — clean execution cannot rescue an undesigned graphic. Never award 8+ while any dimension is ≤4, while any hard-failure field is true, or while you list a reject-class issue.
