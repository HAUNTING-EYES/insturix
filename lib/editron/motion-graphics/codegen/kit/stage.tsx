/**
 * MG Codegen kit — Stage (composition coordinate system). PORTED from explainer-remotion/src/bricks/stage.tsx.
 * MG Codegen Lane E0, §5.
 *
 * ★ LAW 1 CHANGE (the ONE deviation from the source): `backdrop` defaults to FALSE. MG codegen clips
 * render OVER the footage (transparent), not over a brand field — so the Stage must NOT draw the brand
 * backplate by default. `<Stage backdrop={false}>` is the enforced default; the scan (Phase 2b) rejects
 * any `backdrop={true}` for a codegen moment.
 *
 * Every scene mounts ONE <Stage>. It establishes the title-safe region (middle ~90%) and hands children a
 * fraction-based coordinate space, so nothing authored through it can clip: positions are 0..1 of the SAFE
 * region, and text primitives read their Region's pixel width to auto-fit. Imagery that should run
 * edge-to-edge goes in <Bleed> (full frame, BEHIND safe content). Text inside <Bleed> is structurally
 * impossible — the Fit* text primitives throw without a Region context.
 */
...
/** Brand backdrop: bg field + optional editorial grid + gold breathing glow (from brand.decor). Drawn
 *  ONLY when backdrop is explicitly true — off by default for codegen-over-footage (Law 1). */
...
export const Stage: React.FC<{ brand: Brand; backdrop?: boolean; children?: React.ReactNode }> = ({
  brand,
  backdrop = false, // ★ Law 1: transparent by default (MG renders over footage)
  children,
}) => {
