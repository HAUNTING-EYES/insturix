...
import {
  ...
  type MediaSourceAudioAvailabilityEvidenceScopeV1,
...
const DOCUMENT_KIND_V1 =
  'EDITRON_MEDIA_SOURCE_AUDIO_AVAILABILITY_DOCUMENT_V1' as const;
const SCOPE_KIND_V1 =
  'EDITRON_MEDIA_SOURCE_AUDIO_AVAILABILITY_SCOPE_V1' as const;

export const MEDIA_SOURCE_AUDIO_AVAILABILITY_EVIDENCE_COLLECTION_V1 =
  'editron_media_source_audio_availability_evidence_v1' as const;

export interface MediaSourceAudioAvailabilityEvidenceMongoCollectionV1 {
  createIndex(
...
export function createMediaSourceAudioAvailabilityEvidenceMongoPortsV1(
  input: Readonly<{
    loadCollection?: () => Promise<Readonly<
      MediaSourceAudioAvailabilityEvidenceMongoCollectionV1
    >>;
  }> = {},
): Readonly<MediaSourceAudioAvailabilityEvidenceStorePortsV1> {
  ...
  const ensureIndex = async () => {
    indexPromise ??= collection().then(async (resolved) => {
      await resolved.createIndex({
        ...
        name: 'uniq_media_source_audio_availability_scope_v1',
        unique: true,
      });
    });
    try {
  ...
  return Object.freeze({
    ...
    compareAndSet: async (value) => {
      ...
      const next = assertMediaSourceAudioAvailabilityEvidenceV1(value.next);
      if (!sameScope(
        scope,
        mediaSourceAudioAvailabilityEvidenceScopeV1(next),
      )) {
...
>> {
  ...
  return wrapCollection(database.collection(
    MEDIA_SOURCE_AUDIO_AVAILABILITY_EVIDENCE_COLLECTION_V1,
  ));
}
...
async function readStoredRecord(
  collection: MediaSourceAudioAvailabilityEvidenceMongoCollectionV1,
  expectedScope: MediaSourceAudioAvailabilityEvidenceScopeV1,
): Promise<MediaSourceAudioAvailabilityEvidenceV1 | null> {
  ...
  const record = assertMediaSourceAudioAvailabilityEvidenceV1(document.record);
  if (!sameScope(
    mediaSourceAudioAvailabilityEvidenceScopeV1(record),
    expectedScope,
...
function normalizeScope(
  ...
  exactKeys(
    scope,
    ['assetId', 'owner', 'sourceVersionSha256'],
...
function scopeHash(scope: MediaSourceAudioAvailabilityEvidenceScopeV1): string {
...
function sameScope(
  left: MediaSourceAudioAvailabilityEvidenceScopeV1,
  right: MediaSourceAudioAvailabilityEvidenceScopeV1,
): boolean {
...
function fail(code: string): never {
  throw new MediaSourceAudioAvailabilityEvidenceMongoErrorV1(code);
}

export class MediaSourceAudioAvailabilityEvidenceMongoErrorV1 extends Error {
  constructor(code: string) {
    super(`MEDIA_SOURCE_AUDIO_AVAILABILITY_MONGO_${code}`);
    this.name = 'MediaSourceAudioAvailabilityEvidenceMongoErrorV1';
  }
}
