...
export const CHAT_EDIT_PHASE0_BASELINE_CASES: readonly ChatEditBaselineCase[] = [
  ...
  },
  {
    ...
    currentStatus: 'partial-now',
    currentEvidence: 'find_transcript_moment now returns ranked word/frame candidates; still needs real-project proof for cut precision and ambiguity handling.',
  ...
  },
  {
    ...
    currentStatus: 'partial-now',
    currentEvidence: 'find_visual_moment now exposes stored visual analysis candidates; still needs real-project proof for object/OCR coverage.',
  ...
  },
  {
    ...
    currentStatus: 'partial-now',
    currentEvidence: 'find_audio_moment now exposes beat/drop/silence candidates; still needs real-project proof for musical-event recall.',
  ...
  },
  {
    ...
    currentEvidence: 'EDL can apply camera-shake, but chat has no semantic apply_camera_shake tool that resolves target frame, overlay, intensity, and budget.',
  ...
  },
  {
    ...
    currentStatus: 'partial-now',
    currentEvidence: 'set_keyframes can write speed curves, but chat lacks a semantic apply_speed_ramp wrapper that resolves target range and curve safely.',
  ...
  },
  {
    ...
    currentStatus: 'partial-now',
    currentEvidence: 'set_keyframes can write opacity keyframes, but chat lacks a semantic apply_fade wrapper that chooses the range/easing safely.',
  ...
  },
  {
    ...
    currentStatus: 'expected-failure',
    currentEvidence: 'Registry metadata has apply_filter, but no callable chat tool is returned and EDL filter-change is intentionally disabled behind the profile/color owner.',
    ...
    targetPhases: [2, 6, 9, 10],
    requiredContext: ['selected clip or project profile', 'color ownership policy'],
