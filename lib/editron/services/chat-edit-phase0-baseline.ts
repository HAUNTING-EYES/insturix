...
export const CHAT_EDIT_PHASE0_BASELINE_CASES: readonly ChatEditBaselineCase[] = [
  ...
  },
  {
    ...
    currentStatus: 'supported-now',
    currentEvidence: 'resolve_sticker_overlay bridges a transcript word/phrase into generate_html_sticker-ready timing and safe placement params, refusing missing or ambiguous word anchors instead of guessing.',
    ...
    requiredTools: ['find_transcript_moment', 'resolve_sticker_overlay', 'generate_html_sticker'],
  },
  {
  ...
  },
  {
    ...
    currentStatus: 'supported-now',
    currentEvidence: 'resolve_user_asset_overlay searches uploaded assets, refuses low-confidence or ambiguous choices, and returns add_overlay-ready params for a confident uploaded logo/image candidate without generating or mutating assets.',
    ...
    requiredTools: ['search_user_assets', 'resolve_user_asset_overlay', 'add_overlay'],
  },
  {
  ...
  },
  {
    ...
    currentStatus: 'supported-now',
    currentEvidence: 'resolveChatAiEditRestoreTarget deterministically selects the latest safe before/after checkpoint for undo/redo, injects the exact restore_ai_edit_checkpoint target into live chat context, and refuses missing checkpoint targets instead of inverse-editing overlays.',
    successCriteria: ['restores the pre-edit checkpoint', 'shows which edit was reverted', 'does not manually guess inverse edits'],
    targetPhases: [7, 8, 10],
    requiredContext: ['chat checkpoint history', 'before checkpoint id', 'restore target resolver'],
