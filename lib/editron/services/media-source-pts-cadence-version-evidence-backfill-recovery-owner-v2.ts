...
import {
  assertMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryAttemptPolicyV1,
  ...
  resolveMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryLeaseExpiryV1,
  type MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryAttemptPolicyV1,
} from './media-source-pts-cadence-version-evidence-backfill-recovery-attempt-policy-v1';
import {
  createMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoSelectorV1,
  type MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySelectionResultV1,
} from './media-source-pts-cadence-version-evidence-backfill-recovery-mongo-selector-v1';
import {
  createMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreV1,
  type MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepSettlementV1,
} from './media-source-pts-cadence-version-evidence-backfill-recovery-sweep-mongo-store-v1';
import {
...
const RECOVERY_RECEIPT_KIND_V2 =
  'EDITRON_MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_RECEIPT_V2' as const;
const MAX_RECOVERY_SELECTION_LIMIT_V2 = 100;
const MIN_RECOVERY_STALE_MS_V2 =
  2 * MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_DELIVERY_POLICY_V1.timeoutSeconds
    * 1_000;
const MAX_RECOVERY_STALE_MS_V2 = 7 * 24 * 60 * 60 * 1_000;

const RECOVERY_STALE_MS_ENV =
  'EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_STALE_MS' as const;
const RECOVERY_RUN_LIMIT_ENV =
  'EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_RUN_LIMIT' as const;
const RECOVERY_BATCH_LIMIT_ENV =
  'EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_BATCH_LIMIT' as const;
const RECOVERY_MAX_ATTEMPTS_ENV =
  'EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_MAX_ATTEMPTS' as const;
const RECOVERY_LEASE_MS_ENV =
  'EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_LEASE_MS' as const;
const RECOVERY_RETRY_BASE_MS_ENV =
  'EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_RETRY_BASE_MS' as const;
const RECOVERY_RETRY_MAX_MS_ENV =
  'EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_RETRY_MAX_MS' as const;

export const MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_CONTROLLER_ID_V2 =
  'global-pts-cadence-version-evidence-backfill-v1' as const;
export const MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_MIN_LEASE_MS_V2 =
  60_000;

export type MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryEnvironmentV2 =
  MediaSourcePtsCadenceVersionEvidenceBackfillDispatchEnvironmentV1 & Readonly<{
    EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_STALE_MS?: string;
    EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_RUN_LIMIT?: string;
    EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_BATCH_LIMIT?: string;
    EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_MAX_ATTEMPTS?: string;
    EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_LEASE_MS?: string;
    EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_RETRY_BASE_MS?: string;
    EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_RETRY_MAX_MS?: string;
  }>;

export type MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryConfigurationV2 =
  Readonly<{
    staleMs: number;
    selectionLimit: number;
    batchLimit: number;
    attemptPolicy: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryAttemptPolicyV1;
  }>;
...
export type MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryResultV2 = Readonly<{
  migrationRunId: string;
  expectedRecordSha256: string;
  runUpdatedAt: string;
  dispatch: MediaSourcePtsCadenceVersionEvidenceBackfillDispatchResultV1
    | DispatchFailureV2;
}>;
...
type RecoveryReceiptMaterialV2 = Readonly<{
  ...
  results: readonly MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryResultV2[];
}>;

export type MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryReceiptV2 =
  RecoveryReceiptMaterialV2 & Readonly<{
    recoveryReceiptSha256: string;
  }>;

type RecoverySelectorV2 = Pick<
  ReturnType<typeof createMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoSelectorV1>,
  'selectNext'
>;
type RecoverySweepStoreV2 = Pick<
  ReturnType<
    typeof createMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreV1
  >,
  'claimNext' | 'settle'
>;
type DispatcherV2 = typeof dispatchMediaSourcePtsCadenceVersionEvidenceBackfillMessageV1;

export class MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryOwnerErrorV2
  extends Error {
  constructor(public readonly code: string) {
    super('MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_OWNER_V2_' + code);
    this.name = 'MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryOwnerErrorV2';
  }
}

export function resolveMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryConfigurationV2(
  environment: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryEnvironmentV2,
): MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryConfigurationV2 {
    ...
    attemptPolicy:
      createMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryAttemptPolicyV1({
        ...
        leaseMs: requiredInteger(
          ...
          'LEASE_MS',
          MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_MIN_LEASE_MS_V2,
          MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_MAX_LEASE_MS_V1,
        ),
        retryBaseMs,
...
export async function recoverMediaSourcePtsCadenceVersionEvidenceBackfillSweepsV2(
  dependencies: Readonly<{
    environment?: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryEnvironmentV2;
    selector?: Readonly<RecoverySelectorV2>;
    sweepStore?: Readonly<RecoverySweepStoreV2>;
    dispatch?: DispatcherV2;
    publisher?: Readonly<MediaSourcePtsCadenceVersionEvidenceBackfillQStashPublisherV1>;
    now?: Date;
    attemptedAt?: Date;
    claimToken?: string;
  }> = {},
): Promise<MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryReceiptV2> {
  const environment = dependencies.environment ?? processEnvironment();
  const configuration =
    resolveMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryConfigurationV2(environment);
  const dispatchConfiguration =
    resolveMediaSourcePtsCadenceVersionEvidenceBackfillDispatchConfigurationV1(environment);
  ...
  const dispatch = dependencies.dispatch
    ?? dispatchMediaSourcePtsCadenceVersionEvidenceBackfillMessageV1;
  const results: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryResultV2[] = [];
  const attemptResults: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryAttemptResultV1[] = [];
  for (const entry of claim.intent.entries) {
    let delivery: MediaSourcePtsCadenceVersionEvidenceBackfillDispatchResultV1
      | DispatchFailureV2;
    try {
...
function normalizeSelection(
  value: MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySelectionResultV1,
  expected: Readonly<{
    selectedAt: string;
    staleBefore: string;
    selectionLimit: number;
...
): MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepClaimV1 {
...
function normalizeSettlement(
  value: MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepSettlementV1,
  claim: MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepClaimV1,
  attempt: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryAttemptV1,
): SettlementSummaryV2 {
  ...
  const state =
    assertMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateV1(record.state);
...
function createReceipt(input: Readonly<{
  ...
  claim: MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepClaimV1 | null;
  attempt: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryAttemptV1 | null;
  settlement: SettlementSummaryV2 | null;
  results: readonly MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryResultV2[];
}>): MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryReceiptV2 {
...
function processEnvironment(): MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryEnvironmentV2 {
  return {
    ...
    EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_STALE_MS:
      process.env.EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_STALE_MS,
    EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_RUN_LIMIT:
      process.env.EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_RUN_LIMIT,
    EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_BATCH_LIMIT:
      process.env.EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_BATCH_LIMIT,
    EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_MAX_ATTEMPTS:
      process.env.EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_MAX_ATTEMPTS,
    EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_LEASE_MS:
      process.env.EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_LEASE_MS,
    EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_RETRY_BASE_MS:
      process.env.EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_RETRY_BASE_MS,
    EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_RETRY_MAX_MS:
      process.env.EDITRON_MEDIA_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_RETRY_MAX_MS,
  };
}
...
function fail(code: string): never {
  throw new MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryOwnerErrorV2(code);
}
