...
import type { MediaSourceAudioEvidenceBackfillRunLedgerPortsV1 }
...
const RECEIPT_DOCUMENT_KIND_V1 =
  'EDITRON_MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RECEIPT_DOCUMENT_V1' as const;
...
export const MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RUN_COLLECTION_V1 =
  'editron_media_source_audio_evidence_backfill_runs_v1' as const;
export const MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RECEIPT_COLLECTION_V1 =
  'editron_media_source_audio_evidence_backfill_receipts_v1' as const;

export interface MediaSourceAudioEvidenceBackfillMongoSessionV1 {
...
export interface MediaSourceAudioEvidenceBackfillMongoCollectionV1 {
  createIndex(
...
export interface MediaSourceAudioEvidenceBackfillMongoRuntimeV1 {
  startSession(): Promise<MediaSourceAudioEvidenceBackfillMongoSessionV1>;
  runs: MediaSourceAudioEvidenceBackfillMongoCollectionV1;
  receipts: MediaSourceAudioEvidenceBackfillMongoCollectionV1;
}

export function createMediaSourceAudioEvidenceBackfillMongoLedgerPortsV1(
  input: Readonly<{
    loadRuntime?: () => Promise<Readonly<
      MediaSourceAudioEvidenceBackfillMongoRuntimeV1
    >>;
  }> = {},
): Readonly<MediaSourceAudioEvidenceBackfillRunLedgerPortsV1> {
  ...
  const ensureIndexes = async () => {
    indexesPromise ??= runtime().then(async ({ runs, receipts }) => {
      await runs.createIndex(
        { migrationRunId: 1 },
        { name: 'uniq_audio_evidence_backfill_run_v1', unique: true },
      );
      await runs.createIndex(
        { status: 1, updatedAt: 1 },
        { name: 'audio_evidence_backfill_run_status_v1' },
      );
      await receipts.createIndex(
        { batchReceiptSha256: 1 },
        { name: 'uniq_audio_evidence_backfill_receipt_v1', unique: true },
      );
      await receipts.createIndex(
        { migrationRunId: 1, acceptedRecordVersion: 1 },
        { name: 'audio_evidence_backfill_receipt_run_version_v1', unique: true },
      );
    });
    try {
...
function normalizeCasInput(value: Parameters<
  MediaSourceAudioEvidenceBackfillRunLedgerPortsV1['compareAndSet']
>[0]): NormalizedCasInputV1 {
  const next = assertMediaSourceAudioEvidenceBackfillRunRecordV1(value.next);
...
async function applyCompareAndSet(
  runtime: Readonly<MediaSourceAudioEvidenceBackfillMongoRuntimeV1>,
  driverSession: unknown,
  ...
  const derived = input.acceptedReceipt === null
    ? failMediaSourceAudioEvidenceBackfillRunRecordV1(current, {
...
async function retainReceipt(
  collection: MediaSourceAudioEvidenceBackfillMongoCollectionV1,
  driverSession: unknown,
  receipt: MediaSourceAudioEvidenceBackfillBatchReceiptV1,
  acceptedRecord: MediaSourceAudioEvidenceBackfillRunRecordV1,
): Promise<void> {
...
function storedRunDocument(
  recordInput: MediaSourceAudioEvidenceBackfillRunRecordV1,
): Readonly<MongoRecord> {
  const record = assertMediaSourceAudioEvidenceBackfillRunRecordV1(recordInput);
  return Object.freeze({
...
>> {
  ...
  return {
    ...
    receipts: wrapCollection(
      db.collection(MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RECEIPT_COLLECTION_V1),
    ),
  };
}
...
): MediaSourceAudioEvidenceBackfillMongoSessionV1 {
  return {
...
): MediaSourceAudioEvidenceBackfillMongoCollectionV1 {
  return {
...
function fail(code: string): never {
  throw new Error('MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_MONGO_LEDGER_' + code);
}
