...
} from './media-source-pts-cadence-version-evidence-backfill-recovery-attempt-mongo-document-v1';
import {
...
const SWEEP_DUE_INDEX_V1 =
  'pts_cadence_version_evidence_backfill_recovery_sweep_due_v1' as const;
const SWEEP_EXPIRED_LEASE_INDEX_V1 =
  'pts_cadence_version_evidence_backfill_recovery_sweep_expired_lease_v1' as const;
const ATTEMPT_NUMBER_INDEX_V1 =
  'uniq_pts_cadence_version_evidence_backfill_recovery_attempt_number_v1' as const;
const MAX_STORE_ATTEMPTS_V1 = 3;

export interface MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreSessionV1 {
...
export interface MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreCollectionV1 {
  createIndex(
...
export interface MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreRuntimeV1 {
  startSession(): Promise<
    MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreSessionV1
  >;
  sweeps:
    MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreCollectionV1;
  attempts:
    MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreCollectionV1;
}

export type MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepSettlementV1 =
  Readonly<{
    disposition: 'SETTLED' | 'ALREADY_SETTLED';
    state: MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateV1;
  }>;

export class MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreErrorV1
  extends Error {
  constructor(
    ...
    super(
      'MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_SWEEP_MONGO_STORE_'
      + code,
    );
    this.name =
      'MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreErrorV1';
  }
}

export function createMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreV1(
  input: Readonly<{
    loadRuntime?: () => Promise<Readonly<
      MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreRuntimeV1
    >>;
  }> = {},
): Readonly<{
  claimNext(value: Readonly<{
    claimToken: string;
    claimedAt: Date;
  }>): Promise<MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepClaimV1 | null>;
  settle(value: Readonly<{
    sweepIntentSha256: string;
    claimedRecordSha256: string;
    claimToken: string;
    attempt: unknown;
  }>): Promise<MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepSettlementV1>;
  load(
    sweepIntentSha256: string,
  ): Promise<MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateV1 | null>;
}> {
...
async function claimInTransaction(
  ...
  let result: MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepClaimV1
    | null
    | undefined;
  try {
...
async function settleInTransaction(
  ...
  const session = await runtime.startSession();
  let callbackCompleted = false;
  let result: MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepSettlementV1
    | undefined;
  try {
    await session.withTransaction(async () => {
      callbackCompleted = false;
...
async function settleInSnapshot(
  ...
  const next = settleMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateV1(
    current,
  ...
  }, createMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoDocumentV1(next), {
    session: driverSession,
  });
...
async function attemptBelongsToStateChain(
  ...
  driverSession: unknown,
  state: MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateV1,
  target: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryAttemptV1,
): Promise<boolean> {
...
function dueSweepFilter(claimedAt: Date): Readonly<MongoRecord> {
  return Object.freeze({
    schemaVersion: 1,
    kind: MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_SWEEP_DOCUMENT_KIND_V1,
...
function dueSweep(
  ...
  const state =
    parseMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoDocumentV1(value);
...
function race(code: string): never {
  throw new MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepMongoStoreErrorV1(
    code,
    true,
  );
}
