...
import {
  NATIVE_MEDIA_FINAL_RENDER_EXECUTION_BUDGET_LEDGER_RECORD_VERSION_V1,
  type NativeMediaFinalRenderExecutionBudgetLedgerRecordV1,
} from './native-media-final-render-execution-budget-ledger-record-v1';
import type {
  NativeMediaFinalRenderExecutionBudgetLedgerTransactionV1,
  NativeMediaFinalRenderExecutionBudgetLedgerV1,
} from './native-media-final-render-execution-budget-ledger-owner-v1';

type MongoRecord = Record<string, unknown>;

export const NATIVE_MEDIA_FINAL_RENDER_EXECUTION_BUDGET_LEDGER_COLLECTION_V1 =
  'editron_native_media_final_render_execution_budget_ledger_v1' as const;

export interface NativeMediaFinalRenderExecutionBudgetMongoSessionV1 {
...
export interface NativeMediaFinalRenderExecutionBudgetMongoCollectionV1 {
  createIndex(
...
export interface NativeMediaFinalRenderExecutionBudgetMongoRuntimeV1 {
  startSession(): Promise<NativeMediaFinalRenderExecutionBudgetMongoSessionV1>;
  ledger: NativeMediaFinalRenderExecutionBudgetMongoCollectionV1;
}

export function createNativeMediaFinalRenderExecutionBudgetMongoLedgerV1(
  input: Readonly<{
    loadRuntime?: () => Promise<Readonly<
      NativeMediaFinalRenderExecutionBudgetMongoRuntimeV1
    >>;
  }> = {},
): Readonly<NativeMediaFinalRenderExecutionBudgetLedgerV1> {
  ...
  const ensureIndexes = async () => {
    indexPromise ??= runtime().then(async ({ ledger }) => {
      await ledger.createIndex(
        { reservationId: 1 },
        { name: 'uniq_exact_render_execution_budget_reservation_v1', unique: true },
      );
      await ledger.createIndex(
        { 'scope.tenantId': 1, 'scope.projectId': 1, status: 1 },
        { name: 'scope_project_status_exact_render_execution_budget_v1' },
      );
    });
    try {
      await indexPromise;
...
function createTransaction(
  collection: Readonly<NativeMediaFinalRenderExecutionBudgetMongoCollectionV1>,
  session: Readonly<NativeMediaFinalRenderExecutionBudgetMongoSessionV1>,
): Readonly<NativeMediaFinalRenderExecutionBudgetLedgerTransactionV1> {
  return {
    ...
    insert: async (recordInput) => {
      ...
      const stored = await collection.findOneAndUpdate(
        ...
        { session: session.driverSession, upsert: true, returnDocument: 'after' },
      );
    ...
    replace: async ({ expectedRecordSha256, record: recordInput }) => {
      ...
      const replaced = await collection.replaceOne({
        _id: record.reservationId,
        recordSha256: sha256(expectedRecordSha256, 'EXPECTED_RECORD_SHA256'),
        status: 'RESERVED',
        recordVersion: 1,
...
function storedDocument(
  recordInput: Readonly<NativeMediaFinalRenderExecutionBudgetLedgerRecordV1>,
...
function storedRecord(
  ...
  if (document._id !== record.reservationId
    ...
    || dateIso(document.createdAt, 'CREATED_AT') !== record.reservation.reservedAt
...
>> {
  ...
  return {
    ...
    ledger: wrapCollection(
      db.collection(NATIVE_MEDIA_FINAL_RENDER_EXECUTION_BUDGET_LEDGER_COLLECTION_V1),
    ),
  };
}
...
): NativeMediaFinalRenderExecutionBudgetMongoSessionV1 {
  return {
...
): NativeMediaFinalRenderExecutionBudgetMongoCollectionV1 {
  return {
...
function fail(code: string): never {
  throw new NativeMediaFinalRenderExecutionBudgetMongoLedgerErrorV1(code);
}

export class NativeMediaFinalRenderExecutionBudgetMongoLedgerErrorV1 extends Error {
  constructor(code: string) {
    super(`NATIVE_MEDIA_FINAL_RENDER_EXECUTION_BUDGET_MONGO_LEDGER_${code}`);
    this.name = 'NativeMediaFinalRenderExecutionBudgetMongoLedgerErrorV1';
  }
}
