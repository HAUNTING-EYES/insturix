...
import {
  segmentTimedSpeechPhrases,
  type TimedSpeechPhrase,
  type TimedSpeechWord,
} from '@/lib/editron/services/spoken-phrase-segmentation';

type EditDecision = EditDecisionList['decisions'][number];

export type NarrativeBeatWord = TimedSpeechWord;
export type NarrativeBeat = TimedSpeechPhrase;
...
export function segmentNarrativeBeats(words: NarrativeBeatWord[]): NarrativeBeat[] {
  return segmentTimedSpeechPhrases(words, {
    pauseBoundaryMs: 800,
    minimumStandaloneWords: 4,
  });
}
