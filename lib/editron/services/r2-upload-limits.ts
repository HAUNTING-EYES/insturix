 ...
 *  - max object size: 5 TiB (up to 10,000 parts)
 *  - part size: 5 MiB – 5 GiB per part
 ...
 */
export const R2_MAX_OBJECT_BYTES = 5 * 1024 * 1024 * 1024 * 1024; // 5 TiB
export const R2_MIN_PART_BYTES = 5 * 1024 * 1024; // 5 MiB
export const R2_MAX_PART_BYTES = 5 * 1024 * 1024 * 1024; // 5 GiB
 ...
 * while staying inside [5 MiB, 5 GiB]. Throws for files R2 cannot store (> 5 TiB).
 */
export function resolveMultipartPlan(totalBytes: number): MultipartPlan {
  ...
  if (totalParts > R2_MAX_PARTS) {
    ...
    // 5 GiB * 10,000 = 50 TiB > 5 TiB max object, so mathematically impossible.
