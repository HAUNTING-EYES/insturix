...
import { MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RUN_COLLECTION_V1 }
...
import {
  ...
  type MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryControllerV1,
  type MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryOrderCursorV1,
...
} from './media-source-pts-cadence-version-evidence-backfill-recovery-sweep-mongo-document-v1';
import { createMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateV1 }
...
const CONTROLLER_DOCUMENT_KIND_V1 =
  'EDITRON_MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_CONTROLLER_DOCUMENT_V1' as const;
const RUN_RECOVERY_INDEX_V1 = 'pts_cadence_version_evidence_backfill_recovery_v1' as const;
const CONTROLLER_INDEX_V1 =
  'uniq_pts_cadence_version_evidence_backfill_recovery_controller_v1' as const;
const SWEEP_CONTROLLER_VERSION_INDEX_V1 =
  'uniq_pts_cadence_version_evidence_backfill_recovery_sweep_controller_version_v1' as const;
const SWEEP_RETRY_INDEX_V1 =
  'pts_cadence_version_evidence_backfill_recovery_sweep_retry_v1' as const;
...
export const MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_CONTROLLER_COLLECTION_V1 =
  'editron_media_source_pts_cadence_version_evidence_backfill_recovery_controllers_v1' as const;
export interface MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoSessionV1 {
...
export interface MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoCollectionV1 {
  createIndex(
...
export interface MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoRuntimeV1 {
  startSession(): Promise<MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoSessionV1>;
  runs: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoCollectionV1;
  controllers: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoCollectionV1;
  sweeps: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoCollectionV1;
}

export type MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySelectionResultV1 =
  Readonly<
    | {
        disposition: 'NO_CANDIDATES';
        controller: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryControllerV1;
      }
    | {
...
export class MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoSelectorErrorV1
  extends Error {
  constructor(
    public readonly code: string,
    public readonly retryableRace = false,
  ) {
    super('MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_MONGO_' + code);
    this.name = 'MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoSelectorErrorV1';
  }
}

export function createMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoSelectorV1(
  input: Readonly<{
    loadRuntime?: () => Promise<
      Readonly<MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoRuntimeV1>
    >;
  }> = {},
): Readonly<{
  ...
  }>): Promise<MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySelectionResultV1>;
}> {
  const loadRuntime = input.loadRuntime ?? loadDefaultRuntime;
  let runtimePromise: Promise<Readonly<
    MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoRuntimeV1
  >> | null = null;
...
async function selectInTransaction(
  runtime: Readonly<MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoRuntimeV1>,
  input: NormalizedSelectionInputV1,
): Promise<MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySelectionResultV1> {
  const session = await runtime.startSession();
  let callbackCompleted = false;
  let result: MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySelectionResultV1
    | null = null;
  try {
...
async function loadCandidatePage(
  collection: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoCollectionV1,
  driverSession: unknown,
  staleBefore: Date,
  after: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryOrderCursorV1 | null,
  limit: number,
): Promise<readonly MediaSourcePtsCadenceVersionEvidenceBackfillRunRecordV1[]> {
...
function candidateRecord(
  ...
  const record = assertMediaSourcePtsCadenceVersionEvidenceBackfillRunRecordV1(
    document.record,
  );
...
async function insertController(
  ...
  driverSession: unknown,
  controller: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryControllerV1,
): Promise<MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryControllerV1> {
...
function storedControllerDocument(
  input: MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryControllerV1,
): Readonly<MongoRecord> {
  const record = assertMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryControllerV1(
    input,
  );
  return Object.freeze({
...
function assertCandidateOrder(
  records: readonly MediaSourcePtsCadenceVersionEvidenceBackfillRunRecordV1[],
): void {
...
>> {
  ...
  return {
    ...
    runs: wrapCollection(
      db.collection(MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RUN_COLLECTION_V1),
    ),
    controllers: wrapCollection(db.collection(
      MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_CONTROLLER_COLLECTION_V1,
    )),
...
): MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoSessionV1 {
  return {
...
): MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoCollectionV1 {
  return {
...
function retryableSelectionRace(error: unknown): boolean {
  return Boolean(error && typeof error === 'object'
    && ((error as { code?: unknown }).code === 11000
      || (error instanceof
        MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoSelectorErrorV1
        && error.retryableRace)));
}

function race(code: string): never {
  throw new MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoSelectorErrorV1(
    code,
    true,
  );
}
...
function fail(code: string): never {
  throw new MediaSourcePtsCadenceVersionEvidenceBackfillRecoveryMongoSelectorErrorV1(code);
}
