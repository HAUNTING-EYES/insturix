...
export const MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RECOVERY_ATTEMPT_POLICY_KIND_V1 =
  'EDITRON_MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RECOVERY_ATTEMPT_POLICY_V1' as const;
export const MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RECOVERY_MAX_ATTEMPTS_V1 = 20;
export const MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RECOVERY_MIN_LEASE_MS_V1 =
  1_000;
export const MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RECOVERY_MAX_LEASE_MS_V1 =
  24 * 60 * 60 * 1_000;
export const MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RECOVERY_MIN_RETRY_MS_V1 =
  1_000;
export const MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RECOVERY_MAX_RETRY_MS_V1 =
  7 * 24 * 60 * 60 * 1_000;

type AttemptPolicyMaterialV1 = Readonly<{
  schemaVersion: 1;
  kind:
    typeof MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RECOVERY_ATTEMPT_POLICY_KIND_V1;
  maxAttempts: number;
  leaseMs: number;
  retryBaseMs: number;
  retryMaxMs: number;
...
export type MediaSourceAudioEvidenceBackfillRecoveryAttemptPolicyV1 =
...
export class MediaSourceAudioEvidenceBackfillRecoveryAttemptPolicyErrorV1
  extends Error {
  constructor(public readonly code: string) {
    ...
    this.name =
      'MediaSourceAudioEvidenceBackfillRecoveryAttemptPolicyErrorV1';
  }
}

export function createMediaSourceAudioEvidenceBackfillRecoveryAttemptPolicyV1(
  input: Readonly<{
    maxAttempts: number;
    leaseMs: number;
    retryBaseMs: number;
...
): MediaSourceAudioEvidenceBackfillRecoveryAttemptPolicyV1 {
...
export function assertMediaSourceAudioEvidenceBackfillRecoveryAttemptPolicyV1(
  value: unknown,
): MediaSourceAudioEvidenceBackfillRecoveryAttemptPolicyV1 {
...
export function resolveMediaSourceAudioEvidenceBackfillRecoveryLeaseExpiryV1(
  policyValue: unknown,
  claimedAtValue: unknown,
): string {
  const policy =
    assertMediaSourceAudioEvidenceBackfillRecoveryAttemptPolicyV1(policyValue);
...
export function resolveMediaSourceAudioEvidenceBackfillRecoveryRetryAtV1(
  policyValue: unknown,
...
function normalizeMaterial(value: unknown): AttemptPolicyMaterialV1 {
  ...
  const leaseMs = integer(
    record.leaseMs,
    MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RECOVERY_MIN_LEASE_MS_V1,
...
function fail(code: string): never {
  throw new MediaSourceAudioEvidenceBackfillRecoveryAttemptPolicyErrorV1(code);
}
