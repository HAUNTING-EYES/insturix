...
export const CHAT_EDIT_BATTLE_SCENARIOS: readonly ChatBattleScenario[] = [
  ...
  scenario('multiasset-script-intake', 'Multi-asset script with editorial constraints', 'Rebuild this edit from all relevant uploaded footage. Script: First establish the problem. Then show the process in logical order. End on the strongest proof. Keep the speaker understandable and do not add decorative motion graphics.', { requiredToolSequence: [READ_PROJECT, 'apply_editorial_intent'], forbiddenTools: ['auto_edit_from_script'], requireRenderedEvidence: false }),
  scenario('multiasset-script-chat', 'Multi-asset script through chat', 'Use all relevant uploaded footage and reorder it around this script. Script: Open with the core idea. Then show the supporting process and evidence. Finish with the clearest result. Preserve factual order and skip footage that does not support the script.', { requiredToolSequence: [READ_PROJECT, 'apply_editorial_intent'], forbiddenTools: ['auto_edit_from_script'], requireRenderedEvidence: false }),
  ...
  scenario('audio-anchored-camera-shake', 'Audio-anchored camera shake', 'Add one subtle camera shake exactly on the strongest impact beat.', { requiredToolSequence: ['find_audio_moment', 'apply_camera_shake'] }),
  scenario('visual-speed-ramp', 'Visual action speed ramp', 'Speed-ramp only the shot where the fabric is thrown into frame, centered on that action.', { requiredToolSequence: ['find_visual_moment', 'apply_speed_ramp'] }),
  ...
  scenario('analyze-selected-audio', 'Analyze selected clip audio', 'Analyze the selected clip audio for beats, pauses, speech, and energy. Do not edit anything.', { mutationExpectation: 'forbidden', minimumSuccessfulMutations: 0, requiredToolSequence: ['analyze_clip_audio'], requireEvidenceBeforeMutation: false, requireUiReload: false, requireRenderedEvidence: false }),
  scenario('analyze-selected-video', 'Analyze selected clip video', 'Analyze the selected clip visuals for subjects, actions, shot changes, and text. Do not edit anything.', { mutationExpectation: 'forbidden', minimumSuccessfulMutations: 0, requiredToolSequence: ['analyze_clip_video'], requireEvidenceBeforeMutation: false, requireUiReload: false, requireRenderedEvidence: false }),
  ...
  scenario('replace-with-uploaded-footage', 'Replace scene with uploaded footage', 'Find my uploaded embroidery clip and replace the selected video scene with its best matching section.', { requiredToolSequence: ['search_user_assets', 'use_matching_footage'] }),
] as const;
