...
import {
  ...
  resolveMediaSourcePtsCadenceVersionEvidenceBackfillRecoveryRetryAtV1,
...
export const MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_SWEEP_STATE_KIND_V1 =
  'EDITRON_MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_SWEEP_STATE_V1' as const;

export type MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStatusV1 =
  | 'PENDING'
  | 'RUNNING'
  | 'RETRY_WAIT'
  | 'COMPLETE'
...
type SweepStateMaterialV1 = Readonly<{
  schemaVersion: 1;
  kind:
    typeof MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_SWEEP_STATE_KIND_V1;
  ...
  status: MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStatusV1;
  attemptCount: number;
...
export type MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateV1 =
  SweepStateMaterialV1 & Readonly<{
    recordSha256: string;
  }>;

export type MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepClaimV1 = Readonly<{
  sweepIntentSha256: string;
...
export class MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateErrorV1
  extends Error {
  constructor(public readonly code: string) {
    super('MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_SWEEP_STATE_' + code);
    this.name = 'MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateErrorV1';
  }
}

export function createMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateV1(
  intentValue: unknown,
  ...
  return freezeState({
    schemaVersion: 1,
    kind: MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_SWEEP_STATE_KIND_V1,
...
export function claimMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateV1(
  ...
  const current = assertMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateV1(
    currentValue,
  );
...
export function settleMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateV1(
  currentValue: unknown,
  input: Readonly<{
    claimToken: string;
  ...
  const retryAt = attempt.disposition === 'RETRY_REQUIRED'
    ...
    : null;
  const status: MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStatusV1 =
...
export function assertMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateV1(
  value: unknown,
): MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateV1 {
  ...
  if (record.schemaVersion !== 1
    || record.kind
      !== MEDIA_SOURCE_PTS_CADENCE_VERSION_EVIDENCE_BACKFILL_RECOVERY_SWEEP_STATE_KIND_V1
...
function freezeState(
  ...
  return assertMediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateV1({
    ...material,
...
function sweepStatus(
  value: unknown,
): MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStatusV1 {
  if (value !== 'PENDING'
    && value !== 'RUNNING'
    && value !== 'RETRY_WAIT'
    && value !== 'COMPLETE'
...
function fail(code: string): never {
  throw new MediaSourcePtsCadenceVersionEvidenceBackfillRecoverySweepStateErrorV1(code);
}
