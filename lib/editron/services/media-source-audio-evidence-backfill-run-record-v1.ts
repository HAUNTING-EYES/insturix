...
export const MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RUN_KIND_V1 =
  'EDITRON_MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RUN_V1' as const;

export type MediaSourceAudioEvidenceBackfillRunStatusV1 =
  | 'RUNNING'
  | 'COMPLETE'
  | 'COMPLETE_WITH_UNVERIFIABLE'
  | 'FAILED';

export type MediaSourceAudioEvidenceBackfillRunFailureCodeV1 =
  'CANDIDATE_PAGE_INVALID';

export type MediaSourceAudioEvidenceBackfillRunRecordV1 = Readonly<{
  schemaVersion: 1;
  kind: typeof MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RUN_KIND_V1;
  recordVersion: number;
  migrationRunId: string;
  policyVersion: string;
  upperBoundCursor: MediaSourceAudioEvidenceBackfillCursorV1 | null;
  status: MediaSourceAudioEvidenceBackfillRunStatusV1;
  currentCursor: MediaSourceAudioEvidenceBackfillCursorV1 | null;
  ...
  failureCode: MediaSourceAudioEvidenceBackfillRunFailureCodeV1 | null;
  createdAt: string;
  updatedAt: string;
...
export function createMediaSourceAudioEvidenceBackfillRunRecordV1(
  input: Readonly<{
    migrationRunId: string;
    policyVersion: string;
    upperBoundCursor: MediaSourceAudioEvidenceBackfillCursorV1 | null;
  ...
  return freezeRun({
    schemaVersion: 1,
    kind: MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RUN_KIND_V1,
    recordVersion: 1,
...
export function advanceMediaSourceAudioEvidenceBackfillRunRecordV1(
  currentValue: unknown,
  receiptValue: unknown,
): MediaSourceAudioEvidenceBackfillRunRecordV1 {
  const current = assertMediaSourceAudioEvidenceBackfillRunRecordV1(currentValue);
  ...
  const status: MediaSourceAudioEvidenceBackfillRunStatusV1 = terminal
...
export function failMediaSourceAudioEvidenceBackfillRunRecordV1(
  currentValue: unknown,
  input: Readonly<{
...
export function assertMediaSourceAudioEvidenceBackfillRunRecordV1(
  value: unknown,
): MediaSourceAudioEvidenceBackfillRunRecordV1 {
...
function normalizeRunMaterial(value: unknown): RunMaterialV1 {
  ...
  if (record.schemaVersion !== 1
    || record.kind !== MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RUN_KIND_V1
  ...
  const status = record.status as MediaSourceAudioEvidenceBackfillRunStatusV1;
...
function runMaterial(record: MediaSourceAudioEvidenceBackfillRunRecordV1) {
  const { recordSha256: _recordSha256, ...material } = record;
  return material;
}

function freezeRun(material: RunMaterialV1) {
  assertRunSemantics(material);
  return frozen({
    ...material,
...
function nullableFailureCode(
  value: unknown,
): MediaSourceAudioEvidenceBackfillRunFailureCodeV1 | null {
  if (value === null) return null;
...
function fail(code: string): never {
  throw new Error('MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RUN_' + code);
}
