...
export const MEDIA_PROXY_MASTER_TRANSCODE_EXECUTION_BUDGET_SETTLEMENT_VERSION_V2 =
  'EDITRON_MEDIA_PROXY_MASTER_TRANSCODE_EXECUTION_BUDGET_SETTLEMENT_V2' as const;

const IDENTITY = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,239}$/;
const SHA256 = /^[a-f0-9]{64}$/;

export interface MediaProxyMasterTranscodeExecutionBudgetSettlementV2 {
  version:
    typeof MEDIA_PROXY_MASTER_TRANSCODE_EXECUTION_BUDGET_SETTLEMENT_VERSION_V2;
  authority:
    'PROXY_TRANSCODE_PREPARED_PUBLICATION_INTERNAL_COST_SETTLEMENT_NO_CUSTOMER_CHARGE';
...
export function createMediaProxyMasterTranscodeExecutionBudgetSettlementV2(
  input: Readonly<{
    policy: unknown;
...
): Readonly<MediaProxyMasterTranscodeExecutionBudgetSettlementV2> {
  ...
  const terminalEvidenceSha256 = hashEditronCanonicalJsonV1({
    kind: 'PROXY_TRANSCODE_PREPARED_PUBLICATION_BUDGET_TERMINAL_EVIDENCE_V2',
    reservationSha256: reservation.reservationSha256,
    artifactAccountingProfileSha256:
      authorization.scope.artifactAccountingProfileSha256,
    terminalEvidence,
  });
  ...
  const material = {
    version:
      MEDIA_PROXY_MASTER_TRANSCODE_EXECUTION_BUDGET_SETTLEMENT_VERSION_V2,
    authority:
      'PROXY_TRANSCODE_PREPARED_PUBLICATION_INTERNAL_COST_SETTLEMENT_NO_CUSTOMER_CHARGE' as const,
...
export function assertMediaProxyMasterTranscodeExecutionBudgetSettlementV2(
  value: unknown,
  authorization: unknown,
  reservation: unknown,
  policy: unknown,
  ...
  const rebound = createMediaProxyMasterTranscodeExecutionBudgetSettlementV2({
    policy,
    authorization,
    reservation,
