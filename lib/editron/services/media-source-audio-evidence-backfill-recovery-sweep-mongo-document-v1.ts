...
export const MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RECOVERY_SWEEP_DOCUMENT_KIND_V1 =
...
export class MediaSourceAudioEvidenceBackfillRecoverySweepMongoDocumentErrorV1
  extends Error {
  constructor(public readonly code: string) {
    super(
      'MEDIA_SOURCE_AUDIO_EVIDENCE_BACKFILL_RECOVERY_SWEEP_MONGO_DOCUMENT_'
      + code,
    );
    this.name =
      'MediaSourceAudioEvidenceBackfillRecoverySweepMongoDocumentErrorV1';
  }
}

export function createMediaSourceAudioEvidenceBackfillRecoverySweepMongoDocumentV1(
  stateValue: unknown,
): Readonly<MongoRecord> {
  const state = assertMediaSourceAudioEvidenceBackfillRecoverySweepStateV1(
    stateValue,
  );
...
export function parseMediaSourceAudioEvidenceBackfillRecoverySweepMongoDocumentV1(
  value: unknown,
...
function fail(code: string): never {
  throw new MediaSourceAudioEvidenceBackfillRecoverySweepMongoDocumentErrorV1(
    code,
  );
}
