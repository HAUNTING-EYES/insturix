...
if (!gcsCredentials || !process.env.GCS_BUCKET_NAME) {
  console.warn(
    "GCS configuration missing for Clickatron. Image storage will be disabled.",
  );
}
...
export class ClickatronGCSManager {
    ...
    for (let i = 0; i < maxRetries; i++) {
      try {
        ...
        if (isTransient && i < maxRetries - 1) {
          ...
          console.warn(
            `Transient GCS error (${error.code || error.message}). Retrying in ${backoff}ms... (Attempt ${i + 1}/${maxRetries})`,
          );
  ...
  static async uploadImageBuffer(
    ...
    try {
      ...
      // Upload buffer to GCS with retry logic
      ...
      console.error(
        "Failed to upload image to GCS (after retries if applicable):",
        error,
      );
      throw {
  ...
  static async uploadImageFromUrl(
    ...
    try {
      ...
      console.error('Failed to upload image from URL to GCS (after retries):', error);
      throw {
        code: 'GCS_UPLOAD_ERROR',
  ...
  static async uploadThumbnailBuffer(
    ...
    try {
      ...
      console.error('Failed to upload thumbnail to GCS (after retries):', error);
      throw {
        code: 'GCS_UPLOAD_ERROR',
  ...
  static async uploadMaskImage(
    ...
    try {
      ...
      // Upload mask buffer to GCS (always PNG for masks) with retry logic
      await this.withRetry(async () => {
