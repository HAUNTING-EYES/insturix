    ...
    if (microInterval < 1e-6) {
      if (totalSeconds > 1e-9) {
        const zeroMarker: JSX.Element = (
          ...
          >
            <div
              className="h-2 w-[1px] bg-border dark:bg-border/70"
              data-timeline-marker="indicator"
            />
            <span
              className="text-[8px] font-light tracking-tight text-muted-foreground mt-0.5 select-none"
        ...
        >
          <div
            className={`
              ${
                isMajorTick
                  ? "h-2 w-[1px] bg-[hsl(var(--border))]"
                  : isMinorTick
                  ? "h-1 w-px bg-[hsl(var(--border))]/60"
                  : "h-0.5 w-px bg-[hsl(var(--border))]/30"
              }
