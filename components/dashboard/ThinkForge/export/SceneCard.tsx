    ...
    >
      <div className="aspect-video bg-[#1B1A18] relative group">
        {scene.imageUrl ? (
          <>
            ...
            <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1.5">
              <label
                className={`p-1.5 rounded-md bg-emerald-700/80 hover:bg-emerald-600 text-emerald-200 transition-colors cursor-pointer ${isRegenerating ? "opacity-50 pointer-events-none" : ""}`}
          ...
          <div className="w-full h-full flex flex-col items-center justify-center gap-1 text-[#454340]">
            ...
            <button
              onClick={() => handleRegenerateStoryboardScene(scene.sceneIndex)}
              disabled={isRegenerating}
              className="text-[9px] text-[#5CB8CC] hover:text-[#5CB8CC]/80 underline"
            >
        ...
        {isRegenerating && (
          <div className="absolute inset-0 bg-black/70 flex items-center justify-center">
            <Loader2 className="h-5 w-5 text-[#5EC97E] animate-spin" />
          </div>
        )}
      ...
      {showFeedback && (
        <div className="p-1.5 pt-0 space-y-1">
          <textarea
            className="w-full text-[10px] p-1.5 rounded bg-[#1B1A18] border border-[#282724] text-[#ECE9E1] placeholder-[#5F5E5A] resize-none focus:outline-none focus:border-[#5EC97E]"
            rows={2}
          ...
          <button
            ...
            className="w-full text-[10px] py-1 rounded bg-[#5EC97E] hover:bg-[#5EC97E]/80 text-[#0B0B0A] font-medium disabled:opacity-50 transition-colors flex items-center justify-center gap-1"
          >
