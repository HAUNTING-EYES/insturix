...
import { Search, MessageCircle, FileText, Send, Zap, Mail, Phone, MessageSquare, FileQuestion } from "lucide-react";
            ...
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                <div className="absolute inset-0 opacity-[0.02] dark:opacity-[0.05]">
                    {[...Array(5)].map((_, i) => (
                        <motion.div
                            key={i}
                            className="absolute h-[1px] w-full bg-gradient-to-r from-transparent via-blue-500/50 to-transparent"
                            animate={{
                                y: ["0%", "100%"],
                            }}
                            transition={{
                ...
                >
                    ...
                    <div className="grid md:grid-cols-3 gap-6">
                        {quickLinks.map((item, index) => (
                            ...
                            >
                                <Link href={item.link}>
                                    <Card className="p-6 bg-white/50 dark:bg-[rgb(var(--surface-1))]/50 backdrop-blur-sm transition-all duration-300 hover:scale-[1.02] hover:shadow-lg group">
                            ...
                            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                {category.items.map((item) => (
                                    <Link key={item.title} href={item.link}>
                                        <Card className="p-4 bg-white/50 dark:bg-[rgb(var(--surface-1))]/50 backdrop-blur-sm hover:bg-zinc-100/50 dark:hover:bg-zinc-800/50 transition-colors">
