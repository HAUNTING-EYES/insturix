...
import GameOnTeamsManager from "@/components/admin/GameOnTeamsManager";
...
export default function ICS25Dashboard() {
  ...
  const passRevenue = (['bronze','silver','gold','creators'] as TierKey[]).reduce((sum, t) => {
    ...
    return sum + paid * ICS25_PASS_PRICES[t];
  }, 0);
  const gameOnRevenue = (data.gameOn.byPaymentStatus?.paid || 0) * ICS25_GAMEON_PRICE;
                    ...
                    <div className="flex justify-between mt-1">
                      ...
                      <span className="text-xs font-medium text-zinc-700 dark:text-zinc-300">
                        {(() => {
                          ...
                          const price = ICS25_PASS_PRICES[tierKey as TierKey] ?? 0;
                ...
                {Object.entries(data.gameOn.byGame).map(([game, stats]) => (
                  <div key={game}>
                    ...
                    <div className="flex justify-between mt-1">
                      ...
                      <span className="text-xs font-medium text-zinc-700 dark:text-zinc-300">
                        ₹{isNaN(stats.paid * ICS25_GAMEON_PRICE) ? 0 : (stats.paid * ICS25_GAMEON_PRICE).toLocaleString()} revenue
                      </span>
                    </div>
                  </div>
                ))}
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 rounded-lg bg-zinc-50 dark:bg-zinc-900">
                    ...
                    <p className="text-xl font-bold mt-1">₹{isNaN(data.gameOn?.byGame?.valorant?.paid * ICS25_GAMEON_PRICE) ? 0 : (data.gameOn.byGame.valorant.paid * ICS25_GAMEON_PRICE).toLocaleString()}</p>
                  </div>
                  <div className="p-4 rounded-lg bg-zinc-50 dark:bg-zinc-900">
                    <p className="text-sm text-zinc-600 dark:text-zinc-400">BGMI</p>
                    <p className="text-xl font-bold mt-1">₹{isNaN(data.gameOn?.byGame?.bgmi?.paid * ICS25_GAMEON_PRICE) ? 0 : (data.gameOn.byGame.bgmi.paid * ICS25_GAMEON_PRICE).toLocaleString()}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="p-6 rounded-lg border-2 border-green-200 dark:border-green-900 bg-green-50 dark:bg-green-950/20">
                  ...
                  <Link href="/admin/cashback-tasks">
                    <Button className="w-full" variant="outline">
                      Review Cashback Tasks
                      <ExternalLink className="w-4 h-4 ml-2" />
                    </Button>
                  </Link>
                </div>
