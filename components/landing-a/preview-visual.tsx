...
/**
 * PreviewVisual — A/B test variant of the landing page Preview component.
 *
...
const SCRIPT = [
  ...
  { type: "line" as const, text: "Open tight on ice-cold Coca-Cola can. Fast zoom out." },
  { type: "line" as const, text: "VO: Nothing beats a Coke on a summer day." },
  { type: "label" as const, text: "BODY (3-22s)" },
  { type: "line" as const, text: "3 feature callouts: Refreshing Taste, Since 1886, Share a Coke." },
  { type: "line" as const, text: "B-roll: hand reaching for can, condensation close-up, friends sharing." },
  { type: "line" as const, text: "Quote card: Marketing Weekly testimonial at 0:15." },
  { type: "line" as const, text: "Kinetic text overlay, 4 seconds, lower third." },
  { type: "label" as const, text: "CTA (22-30s)" },
  { type: "line" as const, text: "Coca-Cola logo reveal. Open Happiness tagline. End card." },
];
...
function EditVisualFrames({ editSub }: { editSub: number }) {
  return (
    ...
    >
      {/* Frame 1: Product close-up (0.00 - 0.15) */}
      {editSub >= 0 && (
        <div
          style={{
            position: "absolute",
      ...
      {/* Frame 2: Zoom out / wide shot (0.15 - 0.30) */}
      {editSub >= 0.12 && (
        <div
          style={{
            position: "absolute",
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
            <svg width="80" height="100" viewBox="0 0 80 100" fill="none">
              {/* Simplified hand silhouette reaching left */}
              <path
                d="M70 85 C70 75, 65 55, 55 45 L50 40 C48 38, 45 35, 42 33 L38 30 C36 28, 35 25, 38 23 C41 21, 44 23, 45 26 L48 30 L48 22 C48 19, 50 17, 53 17 C56 17, 58 19, 58 22 L58 28 L58 20 C58 17, 60 15, 63 15 C66 15, 68 17, 68 20 L68 30 L68 24 C68 21, 70 19, 73 19 C76 19, 78 21, 78 24 L78 50 C78 65, 75 80, 70 85 Z"
                fill={`${C.soft}18`}
                stroke={`${C.soft}30`}
                strokeWidth="1"
              />
      ...
      {editSub >= 0.27 && (
        ...
        >
          {[
            { text: "Refreshing Taste", delay: 0 },
            { text: "Since 1886", delay: 0.04 },
            { text: "Share a Coke", delay: 0.08 },
          ].map((pill, i) => {
          ...
          >
            SCENE 3 &middot; FEATURE CALLOUTS
          </span>
        </div>
      )}
      ...
      {editSub >= 0.47 && (
        ...
        >
          ...
          >
            ...
            >
              <span style={{ fontSize: 18, color: C.red, fontWeight: 700, fontStyle: "italic" }}>MW</span>
            </div>
            {/* Quote marks */}
            <span style={{ fontSize: 24, color: C.red, lineHeight: 1, opacity: 0.5 }}>&ldquo;</span>
            <span
              style={{
                fontSize: 15,
                color: C.soft,
        ...
        >
          ...
          <span
            style={{
              ...
              textShadow: `0 2px 12px ${C.red}40`,
            }}
          >
            Coca-Cola
          </span>
      ...
      {phase === "prompt" && (
        ...
        >
          <div style={{ textAlign: "center", maxWidth: 420 }}>
            ...
            >
              30-second product launch for Coca-Cola summer campaign
            </p>
          </div>
        </div>
      )}
      ...
      {phase === "edit" && (
        <>
          {editSub < 0.06 && (
            <div
              style={{
                ...
                right: 0,
                height: 2,
                background: `linear-gradient(90deg,transparent,${C.red}50,transparent)`,
          ...
          {editSub >= 0.08 && (
            ...
            >
              <span style={{ fontSize: 20, fontWeight: 800, color: "#fff", letterSpacing: "-0.02em", fontStyle: "italic" }}>Coca-Cola</span>
            </div>
          )}
          {editSub >= 0.28 && (
            <div
            ...
            >
              <div style={{ background: "rgba(0,0,0,.7)", padding: "10px 18px", borderRadius: 8, display: "inline-block" }}>
                <span style={{ color: "#fff", fontSize: 18, fontWeight: 500 }}>
                  Nothing beats an ice-cold <span style={{ color: C.red }}>Coke</span> on a summer day
                </span>
              </div>
            </div>
          )}
          ...
          {editSub >= 0.78 && (
            <div style={{ position: "absolute", top: 20, right: 24, zIndex: 7, animation: `popIn .5s ${EASE} both` }}>
              <div style={{ background: "rgba(0,0,0,.75)", padding: "12px 22px", borderRadius: 12, textAlign: "center" }}>
                <div style={{ fontSize: 32, fontWeight: 800, color: C.red, letterSpacing: "-0.03em" }}>1886</div>
        ...
        >
          <div style={{ width: "88%", maxWidth: 560 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 24 }}>
              ...
              <span style={{ fontSize: 13, color: C.soft, fontWeight: 500 }}>Coca-Cola Summer</span>
            </div>
              ...
              {analyzeSub > 0.88 && (
                <div style={{ animation: `slideR .4s ${EASE} .1s both` }}>
                  <span style={{ fontSize: 18, color: C.text, lineHeight: 1.35 }}>
                    The summer vibe lands instantly.{" "}
                    <span style={{ color: C.red }}>Tighten the tagline reveal timing.</span>
                  </span>
                </div>
              )}
            </div>
              ...
              {THUMBS.map((v, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <span style={{ fontSize: 18, fontWeight: 800, color: `${C.red}12`, fontStyle: "italic" }}>Coca-Cola</span>
                    </div>
      ...
      {phase === "done" && (
        <>
          <div style={{ position: "absolute", top: 20, left: 24, zIndex: 6, background: "rgba(0,0,0,.7)", padding: "8px 20px", borderRadius: 8 }}>
            <span style={{ fontSize: 20, fontWeight: 800, color: "#fff", fontStyle: "italic" }}>Coca-Cola</span>
          </div>
          ...
          <div style={{ position: "absolute", top: 20, right: 24, zIndex: 6 }}>
            <div style={{ background: "rgba(0,0,0,.75)", padding: "12px 22px", borderRadius: 12, textAlign: "center" }}>
              <div style={{ fontSize: 32, fontWeight: 800, color: C.red }}>1886</div>
              <div className="m" style={{ fontSize: 11, color: C.muted }}>EST.</div>
            </div>
          </div>
