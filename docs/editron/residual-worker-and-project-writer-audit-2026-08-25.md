...
| V-JEPA | Step-0 cleanup `e18ff1200` removed one unused request field and raw/debug progress logging while preserving the existing batching, retry and partial-coverage behaviour. Its source endpoint/client pair still uses the former generic `Authorization: Token <MODAL_TOKEN_ID>:<MODAL_TOKEN_SECRET>` pattern. | The required Step-0 boundary is complete; it still needs its own endpoint/caller/exposure audit and bounded fail-closed migration. |

The Scene extractor validates a canonical registration receipt but does not
bind `source.videoUrl` itself to that receipt. Music and Wav2Vec callers
