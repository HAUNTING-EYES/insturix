...
| Direct single-overlay writer | `ProjectService.updateOverlay` | Appends one same-CAS `UPDATE_OVERLAY` receipt with before/after/union timing when both ranges are exact integer project-frame intervals. Existing callers do not yet reliably provide actor provenance, so the receipt honestly records `UNKNOWN_LEGACY_CALLER`. Missing/invalid legacy timing remains stored as `UNKNOWN_LEGACY_OVERLAY_TIMING` and cannot authorize rebase. | A migration of full-family, bulk, audio-worker, manual-state or raw Mongo writers. |
...
range-level undo/proof or dirty-range invalidation authority. A project revision
