...
| `app/api/internal/workers/pipeline/video/route.ts` | Replaces generated-video overlay source/asset fields, adds quality warnings and clears pending Director flags. Commit `3d4852e46` now supplies a focused `ProjectService.commitPipelineVideoDeliveryV1` owner for the replacement only. | The live worker still uses its raw overlay update, so the owner contract is **not wired**. Quality warnings and Director dispatch remain separate raw project-write/dispatch concerns. |
