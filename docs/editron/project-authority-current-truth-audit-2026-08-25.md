...
| `app/api/internal/workers/pipeline/video/route.ts` | Replaces generated-video overlay source/asset fields, adds quality warnings and clears pending Director flags. Commits `3d4852e46` and `6ea12538a` supply a focused `ProjectService.commitPipelineVideoDeliveryV1` owner for the replacement only; it has an exact-target, one-retry safe rebase with requested/applied revision provenance. | The live worker still uses its raw overlay update, so the owner contract is **not wired**. Quality warnings and Director dispatch remain separate raw project-write/dispatch concerns. |
