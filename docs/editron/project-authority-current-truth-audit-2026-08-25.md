...
| `app/api/internal/workers/pipeline/video/route.ts` | Commit `f1a0d3078` makes a project-linked regeneration carry a producer-snapshotted exact target through the signed worker payload; the worker registers the media asset in its existing owner and delegates the overlay replacement to `ProjectService.commitPipelineVideoDeliveryV1`. The receipt has exact-target, one-retry safe rebase and requested/applied revision provenance; conflicts are persisted explicitly on the job. | The raw overlay replacement is removed. Quality warnings and Director dispatch remain separate raw project-write/dispatch concerns. |
