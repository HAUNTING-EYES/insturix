...
The next executable slice is to instantiate `30a2ed776` against its committed
source hashes and run the **zero-inference** model-metadata and official Google
`countTokens` preflight. Only its exact receipt may be used to request paid
dispatch authorization; no inference call is authorised by this matrix update.
