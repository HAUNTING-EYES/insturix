...
Status: **architecture decision plus fresh/resumed PlanService execution adapter, versioned fresh/resumed proof identity, full Plan-to-native cut proof, Plan-lifecycle crash/redelivery/cancellation recovery, fail-closed QStash dispatch, signed worker-adapter contracts, definition-bound execution-owner composition, a production-shaped canonical-media binding/adapter contract and a product-budget reservation/settlement contract plus runtime-guard owner port; zero inference; no concrete canonical-media ports, atomic CreditsService reservation writer, product composition root, live route, live-store exercise or product mutation**
...
| Product workflow ingress/recovery | `0f54a0a2a` provides actor-bound, fixed-URL, message-receipt-backed QStash dispatch; `b6171bed2` provides signed strict worker ingress and refuses to claim without an explicit execution owner; `434563cd6` supplies definition-bound execution composition across accepted episodes; `498e018e6` supplies the canonical-media boundary; `de472b32b` supplies the product-budget contract/runtime-guard boundary. These remain non-routable adapters: concrete canonical-media ports, the atomic CreditsService reservation/partial-settlement writer and locator, root composition, API route export and live Atlas/QStash proof are absent. |
...
- implement the concrete scope-bound canonical-media locator, byte reader and
  rights/privacy policy authority behind the `498e018e6` binding/adapter;
...
- implement the atomic reservation/partial-settlement ledger through the
  existing CreditsService owner and a durable locator for the `de472b32b`
  artifacts; do not treat its contract/port as a wallet implementation;
...
32. **Canonical-media binding/adapter complete at `498e018e6`:** bind native
    ...
    bytes. Concrete locator, byte-reader and policy-owner ports remain open.
...
34. Implement the concrete canonical-media ports and the atomic
    ...
    project authority.
35. Export authenticated routes and run the non-production QStash/Atlas
    crash/restart/redelivery exercise only after step 34 exists.
36. Only after fresh zero-inference preflight and explicit spend approval:
    resumed paid model inference.

## Evidence basis

- Repository code through `6d8fdf1ea` and orchestration-decision commit
  `19d8c97a8`.
