...
| MEDIA-02 | The file-stream upload/registration boundaries are present, therefore long-form reference ingest is streaming. | MISSING_WIRING | False today. `canonicalize-reference.ts` still downloads the whole source into memory before writing a temporary file; `reference-demux.ts` still reads complete source/video/audio files into buffers; and no reference caller invokes `uploadMediaFromFile` or `registerReferenceMaterializedMediaFileV1`. Caller migration remains required. |
