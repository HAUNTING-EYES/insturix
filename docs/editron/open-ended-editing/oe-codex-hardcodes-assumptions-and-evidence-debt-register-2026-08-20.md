...
| SEALED-16 | The historical v1 `buildSubjectAwareReframePlan` treated normalized subject centres as CSS `object-position` percentages and left skipped overlays in old absolute pixels after a canvas change. Commit `0404a253f` replaces that behavior with source-raster-bound, box-aware cover geometry and explicit authored-layout projection in the same sole owner. | OWNER_REPAIRED_PROOF_PENDING | The former owner clipped the bound H05 subject on 38/450 frames (worst 360x640 margin `-20.53px`). The v2 analytical owner test now keeps the supplied box visible and projects the declared top-right-five-percent logo relation; malformed/duplicate layout evidence, missing rasters and impossible crops fail closed. Do not mark H05 PASS or dispatch the paid sealed cohort until a hash-bound adapter decodes the complete 450-frame range. Live chat still lacks canonical authored-layout evidence and ProjectService receipt/undo/replay proof; CAP-2A is stale because this bound owner changed. |
