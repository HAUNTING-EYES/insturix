...
| MEDIA-03 | A successful file upload is a canonical media-registration receipt. | PARTIAL_CONVERGENCE | Still false as a general rule. Commit `54273324d` makes this specific demux owner register exact content-addressed VIDEO/AUDIO children and reject mismatched receipts. Upload and registration remain separate actions, partial failure can leave an object/row requiring reconciliation, and the source envelope does not yet bind the child receipt hashes. Never treat an R2/GCS object alone as canonical evidence. |
