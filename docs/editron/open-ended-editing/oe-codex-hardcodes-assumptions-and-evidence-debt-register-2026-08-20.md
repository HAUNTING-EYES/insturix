...
| MEDIA-03 | A successful file upload is a canonical media-registration receipt. | MISSING_WIRING | Upload and `mediaAssets` registration remain separate owner actions. Commit `a8b7036f9` adds the derived-stream role/provenance contract, but demux does not yet invoke it or return those receipts. Never treat an R2/GCS object alone as canonical evidence. |
