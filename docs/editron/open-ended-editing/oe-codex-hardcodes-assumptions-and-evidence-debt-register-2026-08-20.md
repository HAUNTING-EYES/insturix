...
| MEDIA-02 | The file-stream upload/registration boundaries are present, therefore long-form reference ingest is streaming end to end. | MISSING_WIRING | False today. Commit `7855aa90e` streams the default remote canonical source into a private file and invokes `uploadMediaFromFile` plus `registerReferenceMaterializedMediaFileV1`, but `reference-demux.ts` still reads complete source/video/audio files into Buffers. This proves the default remote canonicalization boundary only, not the full reference pipeline or every caller. |
