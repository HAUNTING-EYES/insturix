...
| ROOT-02 | The composition root is a live product workflow or authorizes canonical project mutation. | MISSING_WIRING | False. The root exports no route, makes no provider call during composition, and has no canonical ProjectService write capability. The next gates are an authenticated signed route, non-production QStash/Atlas/CreditsService crash/redelivery proof, zero-inference preflight and explicit spend approval. Canonical mutation remains disabled after those gates until separately authorized and proven. |
