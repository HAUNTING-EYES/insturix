...
| MEDIA-02 | The file-stream upload/registration boundaries are present, therefore long-form reference ingest is streaming end to end. | MISSING_WIRING | False today. Commits `7855aa90e` and `0917a6c4e` stream the default remote canonical source and demux source/video/audio files, but `reference-frame-sampler.ts` still uses `response.arrayBuffer()` for HTTP media and reads every derived JPEG as a Buffer. The SaaS request-upload seam also starts with a Buffer. These commits prove two boundaries, not the full reference pipeline or every caller. |
