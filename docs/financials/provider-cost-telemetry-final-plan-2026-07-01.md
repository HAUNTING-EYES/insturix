...
- [ ] T6 (P2) - LLM/Search/Social - Record ThinkForge, CalOS, Brand Vault, Alyzitron, UploaderX events.
  ...
  - Partial 2026-07-05: ThinkForge BaseAgent, writing-context cache, prompt-enhance, observer, intent-classifier, filler-repair, thinking-agent, intent-gate fallback, stylist rewrite, post-mortem compression, and search-grounded research provider events are wired for hidden LLM/search spend visibility. BaseAgent records stream success/failure, structured success/failure, and manual JSON fallback success/failure with model/provider, token usage when available, estimated token counts from character counts otherwise, `brandId` as `projectId`, and `sessionId` as `taskId`. The Gemini writing-context cache records cache creation, cached-context completions, and inline-context completions with request count, token usage when available, system-instruction/user-input/output character counts, and cache status. The `/thinkforge/enhance` direct stream records prompt-enhance success from the stream finish callback without blocking the response, `/thinkforge/events/observe` records observer extraction success/failure before downstream databank/embedding work, and the direct helper calls now record intent classification, filler repair, thinking-agent, intent-gate fallback, stylist rewrite, post-mortem compression, and search-grounded research success/failure attempts. Research events include source counts and token/runtime estimates while prompts, generated text, repaired copy, cached document payloads, extracted facts, source URLs, source titles, API tokens, post-mortem input text, brand-event payloads, and error messages stay out of finance metadata. Gemini/OpenRouter-style ThinkForge token pricing and Gemini search-grounding pricing remain `pricing_to_be_seen` until exact model-rate tables are seeded.
  - Partial 2026-07-05: Brand Vault scan provider events are wired for hidden social/visual enrichment spend visibility. Apify public social fallback actor runs now record success/failure events with request/response bytes, runtime, response status, and raw/expanded/accepted/rejected/related item counts; Gemini social/website/upload OCR records image OCR success/failure with image bytes, token usage when available, source kind, platform, media type, and output character counts; committed-thumbnail Gemini vision now records image-vision success/failure with image bytes, token usage when available, output character counts, and parseable-signal status; browser-render fallbacks record endpoint/Modal, local Playwright, and explicit Firecrawl attempts with runtime, response status, request/response bytes, HTML size, stylesheet counts, and rendered-primitive presence. Disabled or missing providers do not create fake spend, and scan outputs, prompts, OCR/vision text, raw HTML, social payloads, URLs, API tokens, and request bodies stay out of finance metadata.
  - Partial 2026-07-05: Brand Vault text-side Gemini enrichment provider events are wired for audience psychographics, approved-copy voice learning, primary text evidence compilation, and malformed-JSON repair compilation. These events record success/failure, runtime, token usage when available, estimated token counts from character counts otherwise, request/response bytes for REST compiler calls, parseable-output flags, accepted/rejected candidate counts, and repair usage without storing prompts, source text, source URLs, model output, request bodies, API tokens, payloads, or error messages. Apify actor, Gemini OCR/vision/text enrichment, Modal/local Playwright, and Firecrawl pricing remain `pricing_to_be_seen` until invoice-backed rates are added.
  ...
  - Partial 2026-07-05: UploaderX legacy GCS upload route provider events are wired for `youtube-data-api` cost visibility from `app/api/services/uploaderx/youtube/upload/route.ts`. This deployed but currently unreferenced route records `videos.insert` success/failure once the provider attempt starts, stores request count, runtime, response status, provider video ID, and sanitized source flags, and deliberately attaches no charged credits because the route has no credit deduction boundary. Titles, descriptions, legacy user references, GCS paths, OAuth tokens, request bodies, and stream payloads stay out of finance metadata. YouTube Data API legacy upload-route pricing remains `pricing_to_be_seen` until invoice-backed rates are added.
  - Partial 2026-07-05: UploaderX YouTube chunk-route provider events are wired for `youtube-data-api` cost visibility. Chunked resumable uploads now record start, transfer, finish, and thumbnail success/failure events from `app/api/services/uploaderx/youtube/chunk/route.ts`; transfer events include input bytes and final upload status while upload locations and OAuth details stay sanitized. Final chunk publish revenue attaches the credit transaction ID and the current 1-credit YouTube publish charge only after credit deduction returns a transaction ID. Successful provider upload is still recorded without fake revenue if later DB/event/credit work fails. YouTube Data API chunk-route pricing remains `pricing_to_be_seen` until invoice-backed rates are added.
  - Partial 2026-07-04: UploaderX Facebook direct-route provider events are wired for `meta-graph-api` cost visibility. Existing-video updates record publish update success/failure; small direct uploads record simple-upload success/failure; fallback resumable video/Reel uploads record start/transfer/finish success/failure. Final Facebook publish revenue attaches the credit transaction ID and the current 1-credit Facebook publish charge only after credit deduction returns a transaction ID. LinkedIn main and chunk routes are now wired. Meta Graph API pricing remains `pricing_to_be_seen` until invoice-backed rates are added.
  ...
  - Partial 2026-07-05: UploaderX Instagram main-route provider events are wired for `instagram-graph-api` cost visibility. The main route now records media container creation, large-video chunk transfers, video status polling, and final media_publish success/failure events from `app/api/services/uploaderx/instagram/route.ts`; chunk transfers include input bytes and poll events include request counts. Final Instagram publish revenue attaches the credit transaction ID and the current 1-credit Instagram publish charge only after credit deduction returns a transaction ID. Successful provider publish is still recorded without fake revenue if later DB/event/credit work fails. Instagram Graph API pricing remains `pricing_to_be_seen` until invoice-backed rates are added.
  - Partial 2026-07-05: UploaderX Instagram chunk-route provider events are wired for `instagram-graph-api` cost visibility. The separate chunk route now records container creation, chunk transfer, status poll, and final media_publish success/failure events from `app/api/services/uploaderx/instagram/chunk/route.ts`; transfer events include input bytes and poll events include provider status codes while provider identifiers stay sanitized. Final chunk publish revenue attaches the credit transaction ID and the current 1-credit Instagram publish charge only after credit deduction returns a transaction ID. Successful provider publish is still recorded without fake revenue if later DB/event/credit work fails. Instagram Graph API chunk-route pricing remains `pricing_to_be_seen` until invoice-backed rates are added.
  - Partial 2026-07-05: UploaderX LinkedIn main-route provider events are wired for `linkedin-api` cost visibility. The main route now records REST video initialize/transfer/finalize, REST image/document initialize/transfer, legacy asset register/upload, and final post_create success/failure events from `app/api/services/uploaderx/linkedin/route.ts`; transfer events include input bytes and provider status codes while upload locations, OAuth details, organization IDs, and post content stay out of finance metadata. Final LinkedIn publish revenue attaches the credit transaction ID and the current 1-credit LinkedIn publish charge only after credit deduction returns a transaction ID. Successful provider post creation is still recorded without fake revenue if later DB/event/credit work fails. LinkedIn API main-route pricing remains `pricing_to_be_seen` until invoice-backed rates are added.
  - Partial 2026-07-05: UploaderX LinkedIn chunk-route provider events are wired for `linkedin-api` cost visibility. The chunk route now records initialize, transfer, finalize, and final post_create success/failure events from `app/api/services/uploaderx/linkedin/chunk/route.ts`; transfer events include input bytes and chunk offsets while upload URLs, OAuth details, organization IDs, and post content stay out of finance metadata. Final chunk publish revenue attaches the credit transaction ID and the current 1-credit LinkedIn publish charge only after credit deduction returns a transaction ID. Successful provider post creation is still recorded without fake revenue if later DB/event/credit work fails. LinkedIn API chunk-route pricing remains `pricing_to_be_seen` until invoice-backed rates are added.
  - Partial 2026-07-05: CalOS planner and trends provider events are wired for hidden calendar-planning/search/scrape spend visibility. Gemini AI planner calls now record success/failure events with request count, runtime, token usage when available, estimated token counts from character counts otherwise, slot/trend/platform counts, result count, objective, and boolean brand-context flags; Apify actor runs now record success/failure events with request count, request bytes, runtime, response status, and result counts; Perplexity Sonar trend searches record request count, request bytes, runtime, response status, and token usage when the API returns it; Gemini grounded-search trend calls record request count, runtime, response characters, result counts, and Gemini usage metadata when available. Trend events attach `brandId` as `projectId` and intentionally do not attach charged credits because trends can be a hidden input to AI planning or an on-demand discovery view rather than a separately billed customer action. Planner prompts, brand context text, niche text, API tokens, provider payloads, and source URLs stay out of finance metadata. Apify, Perplexity Sonar, and Gemini grounded-search pricing remain `pricing_to_be_seen` until invoice-backed rates are added.
  - Partial 2026-07-05: CalOS Facebook, Instagram, LinkedIn, and YouTube publisher events are wired for publish-queue platform spend visibility. Facebook Page feed posts, Instagram image container creation plus `media_publish`, LinkedIn REST posts, and YouTube Data API video inserts now record publish success/failure, provider job IDs when available, request count, response status when available, retryability, brand/account-ref flags, and sanitized platform metadata. Publisher-level events intentionally do not attach charged credits because the customer charge belongs to the upstream CalOS workflow, not a hidden cron retry; captions, post text, titles, descriptions, image/video URLs, OAuth tokens, public post URLs, and provider payloads stay out of finance metadata. Meta Graph, Instagram Graph, LinkedIn, and YouTube Data API pricing remain `pricing_to_be_seen` until invoice-backed rates are added.
  - Partial 2026-07-05: Editron transcription provider events are wired for xAI/Grok STT, Fal Wizper, Gemini/Gemma transcription, and Deepgram fallback/direct-wrapper attempts. The media transcription orchestrator now records xAI/Grok, Fal, and Gemini success/failure attempts with request counts, retry counts where applicable, runtime, byte counts, media seconds when returned, word/segment/speaker counts, and sanitized asset flags; the Deepgram wrapper records both fallback calls from media transcription and direct calls from five-track analysis with source mode, duration, bytes, language, confidence, and word counts. Synthetic narration timing still creates no fake provider spend, download/API-key failures before a provider attempt still create no fake spend, and media URLs, signed URLs, prompts, transcripts, provider payloads, OAuth/API keys, and request bodies stay out of finance metadata. xAI/Grok STT, Fal Wizper, Gemini transcription, and Deepgram pricing remain `pricing_to_be_seen` until invoice-backed rates are added.
  ...
  - Partial 2026-07-05: Alyzitron Apify media extraction provider events are wired for hidden pre-analysis extraction spend visibility. Apify actor starts, polls, and dataset fetches now record success/failure with request count, request bytes, poll count, runtime, actor ID as model, provider run ID when available, platform, response statuses, final run status, item count, media type, and sanitized media-presence flags. Missing `APIFY_API_KEY` still creates no fake spend, and source URLs, extracted media URLs, actor input bodies, API tokens, dataset payloads, and error messages stay out of finance metadata. Apify media-extraction pricing remains `pricing_to_be_seen` until invoice-backed rates are added.
