...
- [ ] T6 (P2) - LLM/Search/Social - Record ThinkForge, CalOS, Brand Vault, Alyzitron, UploaderX events.
  ...
  - Partial 2026-07-05: CalOS Facebook, LinkedIn, and YouTube publisher events are wired for publish-queue platform spend visibility. Facebook Page feed posts, LinkedIn REST posts, and YouTube Data API video inserts now record publish success/failure, provider job IDs when available, request count, response status when available, retryability, brand/account-ref flags, and sanitized platform metadata. Publisher-level events intentionally do not attach charged credits because the customer charge belongs to the upstream CalOS workflow, not a hidden cron retry; captions, post text, titles, descriptions, video URLs, OAuth tokens, public post URLs, and provider payloads stay out of finance metadata. Meta Graph, LinkedIn, and YouTube Data API pricing remain `pricing_to_be_seen` until invoice-backed rates are added.
