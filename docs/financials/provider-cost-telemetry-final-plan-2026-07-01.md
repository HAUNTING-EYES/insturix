...
- [ ] T6 (P2) - LLM/Search/Social - Record ThinkForge, CalOS, Brand Vault, Alyzitron, UploaderX events.
  ...
  - Partial 2026-07-05: ThinkForge BaseAgent, writing-context cache, prompt-enhance, observer, intent-classifier, filler-repair, thinking-agent, intent-gate fallback, stylist rewrite, and post-mortem compression provider events are wired for hidden LLM spend visibility. BaseAgent records stream success/failure, structured success/failure, and manual JSON fallback success/failure with model/provider, token usage when available, estimated token counts from character counts otherwise, `brandId` as `projectId`, and `sessionId` as `taskId`. The Gemini writing-context cache records cache creation, cached-context completions, and inline-context completions with request count, token usage when available, system-instruction/user-input/output character counts, and cache status. The `/thinkforge/enhance` direct stream records prompt-enhance success from the stream finish callback without blocking the response, `/thinkforge/events/observe` records observer extraction success/failure before downstream databank/embedding work, and the direct helper calls now record intent classification, filler repair, thinking-agent, intent-gate fallback, stylist rewrite, and post-mortem compression success/failure attempts. Prompts, generated text, repaired copy, cached document payloads, extracted facts, URLs, API tokens, post-mortem input text, brand-event payloads, and error messages stay out of finance metadata. Gemini/OpenRouter-style ThinkForge token pricing remains `pricing_to_be_seen` until exact model-rate tables are seeded.
