...
- [ ] T6 (P2) - LLM/Search/Social - Record ThinkForge, CalOS, Brand Vault, Alyzitron, UploaderX events.
  ...
  - Partial 2026-07-05: Brand Vault scan provider events are wired for hidden social/visual enrichment spend visibility. Apify public social fallback actor runs now record success/failure events with request/response bytes, runtime, response status, and raw/expanded/accepted/rejected/related item counts; Gemini social/website/upload OCR records image OCR success/failure with image bytes, token usage when available, source kind, platform, media type, and output character counts; browser-render fallbacks record endpoint/Modal, local Playwright, and explicit Firecrawl attempts with runtime, response status, request/response bytes, HTML size, stylesheet counts, and rendered-primitive presence. Disabled or missing providers do not create fake spend, and scan outputs, prompts, OCR text, raw HTML, social payloads, URLs, API tokens, and request bodies stay out of finance metadata. Apify actor, Gemini OCR, Modal/local Playwright, and Firecrawl pricing remain `pricing_to_be_seen` until invoice-backed rates are added.
