...
- [ ] T6 (P2) - LLM/Search/Social - Record ThinkForge, CalOS, Brand Vault, Alyzitron, UploaderX events.
  ...
  - Partial 2026-07-05: ThinkForge BaseAgent and writing-context cache provider events are wired for hidden LLM spend visibility. BaseAgent now records stream success/failure, structured success/failure, and manual JSON fallback success/failure with model/provider, token usage when available, estimated token counts from character counts otherwise, `brandId` as `projectId`, and `sessionId` as `taskId`. The Gemini writing-context cache now records cache creation, cached-context completions, and inline-context completions with request count, token usage when available, system-instruction/user-input/output character counts, and cache status. Prompts, generated text, cached document payloads, URLs, API tokens, and error messages stay out of finance metadata. Gemini/OpenRouter-style ThinkForge token pricing remains `pricing_to_be_seen` until exact model-rate tables are seeded.
