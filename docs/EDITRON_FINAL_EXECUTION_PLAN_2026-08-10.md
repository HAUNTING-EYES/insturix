...
programme code checkpoint represented by this ledger is `372001cc1` on
...
over a serialized test record. It is **not** live-infrastructure convergence:
non-production Atlas persistence, QStash delivery/redelivery, authenticated
worker ingress, real ProjectService clone/execution, secret-bearing provider
transport, paid provider resume and rendered acceptance remain unchecked.
...
`fee52df57` adds strict owner coordination and artifact revalidation. Real
episode-definition, ProjectService clone/execution, reference-media, runtime-
budget and provider-transport adapters, authenticated ingress/QStash dispatch,
live-Atlas/QStash suffix recovery, paid resumed inference and
...
3. **Stage 2.5 generalisation episodes:** the eight HOLD tasks are no longer
   ...
   inference. Next bind
   the real episode-definition, ProjectService clone/execution,
   reference-media, runtime-budget and provider-transport adapters; add
