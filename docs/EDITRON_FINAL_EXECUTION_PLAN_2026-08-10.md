...
3. **Stage 2.5 generalisation episodes:** the eight HOLD tasks are no longer
   ...
   `$1.5051264` absolute ceiling. Next instantiate it against the committed
   source hashes and run the zero-inference request/token/spend preflight; only
   after its exact receipt and explicit authorization measure model planning quality under
