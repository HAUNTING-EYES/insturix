...
programme code checkpoint represented by this ledger is `9687dbd9f` on
...
| Stage 2 - scalable ingest, media identity, evidence and durable jobs | Upload, proxy, transcription, several analyzers, R2/Mongo/Qdrant pieces and job mechanisms exist in separate paths. The shared durable chain now comprises the Mongo lifecycle record (`1f22ef2f2`), V4 checkpoint binding (`96d2e3923`), terminal-writer revision repair (`ecfa20c54`), zero-inference recovery worker (`81ce63d9b`), strict artifact coordination (`fee52df57`), separate-process captured suffix proof (`372001cc1`), exact provider-route transport (`2bdb05d80`), serialized manifest-bound research episode definition (`e3ac9b082`), the non-wired immutable product PlanService store (`0c94bc059`) and exact accepted-node-to-job binding (`9687dbd9f`). | There is no converged long-form media identity/evidence contract. Source cadence/PTS, VFR/CFR mapping, timecode/reel identity, colour/audio metadata, shared invalidation, bounded dense inspection and sharded/resumable proof are incomplete. Execution-time PlanService/artifact revalidation, ProjectService clone/execution, reference-media and runtime-budget adapters, authenticated ingress/QStash dispatch, live Atlas/QStash proof, event history, approval waits and family-store migration remain. |
...
`PlanService` revision/definition store now exists. Commit `9687dbd9f` also
binds an exact accepted product definition into the shared job input. The
research recovery worker does not yet consume that product binding, revalidate
its expected plan head or resolve all referenced artifacts, and there is no
...
3. **Stage 2.5 generalisation episodes:** the eight HOLD tasks are no longer
   ...
   Commit `0c94bc059` implements the immutable Mongo PlanService store and
   `9687dbd9f` binds one accepted node/definition into the shared job input.
   Next add execution-time PlanService/artifact revalidation and prove zero-
   inference crash/restart plus redelivery; then bind ProjectService clone/execution,
