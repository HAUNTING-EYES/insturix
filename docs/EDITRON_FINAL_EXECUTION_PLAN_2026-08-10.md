...
programme code checkpoint represented by this ledger is `f85bc0f09` on
...
<!-- CURRENT RESUME CHECKPOINT: f85bc0f09. Deterministic proposal recovery, one
real cut_section owner, immutable reference/runtime-budget owners and strict
outcome-proof completion mechanics are research-proven. Next connect a real
Remotion/Phase-0 proof producer and expand concrete operator coverage; do not
rerun paid cohorts without fresh zero-inference preflight plus explicit spend
authorization. -->
...
test owner. It is not rendered proof. The next slice must adapt the existing
Phase-0/Remotion producer to the isolated proposal, bind a real versioned proof
policy and prove rendered artifacts; until then a changed production proposal
must remain ineligible for apply.
