...
programme code checkpoint represented by this ledger is `b9cf5e820` on
...
| Stage 2 - scalable ingest, media identity, evidence and durable jobs | Upload, proxy, transcription, several analyzers, R2/Mongo/Qdrant pieces and job mechanisms exist in separate paths. The shared durable chain now comprises the Mongo lifecycle record (`1f22ef2f2`), V4 checkpoint binding (`96d2e3923`), terminal-writer revision repair (`ecfa20c54`), zero-inference recovery worker (`81ce63d9b`), strict artifact coordination (`fee52df57`), separate-process captured suffix proof (`372001cc1`), exact provider-route transport (`2bdb05d80`), serialized manifest-bound research episode definition (`e3ac9b082`), the non-wired immutable product PlanService store (`0c94bc059`), exact accepted-node-to-job binding (`9687dbd9f`), fresh-instance execution-time revalidation (`d16caaa5b`) and serialized separate-process product recovery (`b9cf5e820`). | There is no converged long-form media identity/evidence contract. Source cadence/PTS, VFR/CFR mapping, timecode/reel identity, colour/audio metadata, shared invalidation, bounded dense inspection and sharded/resumable proof are incomplete. Complete artifact resolution, lifecycle approval/cancel/expiry/tenant proof, ProjectService clone/execution, reference-media and runtime-budget adapters, authenticated ingress/QStash dispatch, live Atlas/QStash proof, event history and family-store migration remain. |
...
3. **Stage 2.5 generalisation episodes:** the eight HOLD tasks are no longer
   ...
   approval wait/cancel/expiry/tenant-isolation coverage, bind ProjectService
