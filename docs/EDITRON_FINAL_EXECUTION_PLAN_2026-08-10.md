...
| **1-B2d** | **in progress** (`abeaf6a81`) | A server-issued attempt token now binds the exact checkpoint/session/operation/after-checkpoint proof identity across checkpoint and derived ProjectService projection CAS writes.  A superseded worker cannot complete a replacement attempt.  Remaining: checkpoint-owned dispatch lease/recovery for a crash or ambiguous queue publish, plus idempotent terminal-notification recovery. |
