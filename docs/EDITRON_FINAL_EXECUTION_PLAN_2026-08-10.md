...
- **The generic sealed-holdout episode shell is `CODE_CONNECTED`; owner evidence,
  lowering and evaluation remain unwired.** Commit
  ...
  shell still has an injected isolated executor and cannot yet resolve the
  separately hashed owner-only evidence or judge an outcome. It grants no
  ProjectService, provider-dispatch or production authority. The related
  protocol/preflight suite passes 25/25; typecheck and repository lint pass.
- **Stage 2.5 status remains `MODIFY_AND_PROCEED_RESEARCH`.** No production
  ...
  has been dispatched. The exact immediate order is now: connect owner-evidence
  handoff, the isolated executor, schema-driven lowering and the evaluator to
  the generic frozen-cohort episode; capture and token-bound the real requests
...
| Stage 2.5 - open-ended planner experiment | Provider-native sequential episodes, typed results, isolated native/generated/hybrid mechanics and rendered proof exist. V3R4 is reproducibly landed and valid bounded evidence: all 18 Luna/Terra/Gemini rows passed causal execution, direct/opaque writer-revision handoff, rendered proxy proof and no-mutation under three tool orders. CAP-2A V3 is current. HREF-01 has one complete Gemini 3.6 native-video/audio observation and a blinded full-reference plus 180-frame dense-window review pack. All eight sealed holdouts have deterministic, rights-bound media inputs, and their leakage-free 16-case/40-operation local preflight passes. The generic public-case episode shell is connected with a distinct policy-blocked outcome; no sealed holdout has been dispatched. One project-owner blind review accepted the existing DEV-01/DEV-03 proxies. | Result is `MODIFY_AND_PROCEED_RESEARCH`, not production `GO`. HREF-01 output review is pending and formal promotion requires a second independent qualified reviewer. Connect owner-only evidence handoff, the isolated executor, schema-driven lowering/evaluation and credentialed request/token preflight, then complete all eight sealed unseen holdouts, different dependency shapes, forced routing alternatives, stale-user-edit/conflict/rebase, context-resume, long-form trials and blind quality/correction-time/latency/cost receipts. H04 uses synthetic tone audio and DEV-03 does not contain intelligible speech. |
...
3. **V2-2 unseen holdouts and integration gate - generic public episode
   connected, dispatch zero:** all eight sealed unseen holdouts now have frozen,
   ...
   semantics. Next connect owner-only evidence, the isolated executor,
   schema-driven lowering/evaluation and run the credentialed
