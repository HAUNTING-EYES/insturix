...
- **2026-08-25 PTS/cadence mapping Step-0.** The next canonical-media work is
  ...
  No signed durable worker claims, dispatches,
  polls or resumes the job; no execution-time source URL is issued; no finalizer
  is invoked by the durable lifecycle; and no Modal/R2 or Atlas deployment proof
  was created. The versioned 8 MiB/50,000-record scan bounds, 8 MiB/10,000-entry
  ...
  absent. Signed worker wiring is next; DEP-02/DEP-03, project routing and model
  evaluation remain later in the frozen order.
