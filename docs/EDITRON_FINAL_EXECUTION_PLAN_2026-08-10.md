...
programme code checkpoint represented by this ledger is `2bdb05d80` on
...
real episode-definition, ProjectService clone/execution, reference-media and
runtime-budget adapters plus ingress/dispatch remain unwired. No live Atlas or
QStash recovery and no paid resumed inference were exercised.
...
| Stage 2 - scalable ingest, media identity, evidence and durable jobs | Upload, proxy, transcription, several analyzers, R2/Mongo/Qdrant pieces and job mechanisms exist in separate paths. Commit `1f22ef2f2` adds one shared transport-neutral Mongo lifecycle record/store with immutable input binding, idempotency, leases, retries, cancellation, resume-state CAS, dispatch metadata, budget binding and terminal proof references. Commit `96d2e3923` binds the existing research V4 provider checkpoint into that record; `ecfa20c54` binds terminal writer revisions; `81ce63d9b` exercises the transport-neutral zero-inference recovery core over fresh store/worker instances; `fee52df57` verifies owner-coordinated, hash-checked artifact resolution without adding storage; `372001cc1` proves exact captured-trace suffix recovery across two separate Node processes over a serialized Mongo-shaped record; and `2bdb05d80` binds the existing live HTTP transport to one exact durable route with provider-scoped credentials and request/response model identity checks. | There is no converged long-form media identity/evidence contract. Source cadence/PTS, VFR/CFR mapping, timecode/reel identity, colour/audio metadata, shared invalidation, bounded dense inspection and sharded/resumable proof are incomplete. No authenticated production ingress or transport executes through the shared record; real episode-definition, ProjectService-clone/execution, reference-media and runtime-budget adapters, event history, approval waits, live Atlas/QStash proof and family-store migration remain. |
...
episode-definition, ProjectService clone/execution, reference-media and runtime-
budget adapters, authenticated ingress/QStash dispatch,
...
3. **Stage 2.5 generalisation episodes:** the eight HOLD tasks are no longer
   ...
   exact durable route without requiring an unrelated provider secret. Next bind
