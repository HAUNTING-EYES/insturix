...
programme code checkpoint represented by this ledger is `a9882903a` on
...
This closes identity separation only. The ProjectService adapter currently
creates a fresh clone whose working identity equals its canonical base. After
a crash following an isolated mutation, that fresh clone intentionally fails
the resume writer-revision check instead of pretending that prior proposal
state was restored. Deterministic restart-safe proposal recovery is therefore
the next P0 gate before concrete product-owner wiring, live Atlas/QStash work
or paid resumed inference.
...
is retired. Concrete production operator ownership, reference-media and
...
revision/state from the isolated proposal working revision/state. It prevents
an honest ProjectService read from being relabelled as the latest local writer
revision, but deliberately does not claim that proposal state survives process
loss. Restart-safe isolated-proposal recovery now precedes all concrete owner,
live infrastructure and paid-resume work.
...
3. **Stage 2.5 generalisation episodes:** the eight HOLD tasks are no longer
   ...
   `a9882903a` separates canonical-base and isolated-working identities. Next
   restore the isolated proposal deterministically after process loss; only
   then bind concrete production operator ownership, reference-media and runtime-budget
   adapters, and add
