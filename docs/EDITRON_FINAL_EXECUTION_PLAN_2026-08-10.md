...
3. **Stage 2.5 generalisation episodes:** the eight HOLD tasks are no longer
   ...
   PlanService without inference or project effects. Next issue its immutable
   provider-cohort identity, run a zero-inference request/token/spend preflight,
   and only after explicit authorization measure model planning quality under
