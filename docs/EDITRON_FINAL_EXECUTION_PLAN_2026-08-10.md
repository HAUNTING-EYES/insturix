...
programme code checkpoint represented by this ledger is `d16caaa5b` on
...
| Stage 2 - scalable ingest, media identity, evidence and durable jobs | Upload, proxy, transcription, several analyzers, R2/Mongo/Qdrant pieces and job mechanisms exist in separate paths. The shared durable chain now comprises the Mongo lifecycle record (`1f22ef2f2`), V4 checkpoint binding (`96d2e3923`), terminal-writer revision repair (`ecfa20c54`), zero-inference recovery worker (`81ce63d9b`), strict artifact coordination (`fee52df57`), separate-process captured suffix proof (`372001cc1`), exact provider-route transport (`2bdb05d80`), serialized manifest-bound research episode definition (`e3ac9b082`), the non-wired immutable product PlanService store (`0c94bc059`), exact accepted-node-to-job binding (`9687dbd9f`) and fresh-instance execution-time revalidation (`d16caaa5b`). | There is no converged long-form media identity/evidence contract. Source cadence/PTS, VFR/CFR mapping, timecode/reel identity, colour/audio metadata, shared invalidation, bounded dense inspection and sharded/resumable proof are incomplete. Serialized separate-OS-process product recovery, complete artifact resolution, ProjectService clone/execution, reference-media and runtime-budget adapters, authenticated ingress/QStash dispatch, live Atlas/QStash proof, event history, approval waits and family-store migration remain. |
...
fresh store instances. The research recovery worker is still a separate
research path, all referenced product artifacts are not yet resolved, and the
product binding has not survived a real process exit or live Atlas exercise.
ProjectService clone/execution, reference-media and runtime-budget adapters,
authenticated ingress/QStash dispatch, live-Atlas/QStash suffix recovery,
paid resumed inference and ProjectService integration also remain open.
...
3. **Stage 2.5 generalisation episodes:** the eight HOLD tasks are no longer
   ...
   `d16caaa5b` revalidates the leased job through fresh store instances with
   stale-lease and duplicate-delivery rejection. Next prove that product
   binding across a serialized separate-OS-process restart; then add approval
   wait/cancel/expiry/tenant-isolation coverage, bind ProjectService
   clone/execution, reference-media and runtime-budget adapters, and add
