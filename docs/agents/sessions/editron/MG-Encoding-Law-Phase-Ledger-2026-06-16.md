...
- Regression coverage: `tests/editron/mg-stage-composition-renderer.test.ts` proves licensed `90%` content still gets stat atoms and unbounded scalar-rate content gets none. Broader MG/render-validity tests passed after the fix.
- Still needed: rendered-frame proof and taste review for the live screenshot class; rate/tiny-magnitude stats may still need a better licensed atom treatment rather than only suppressing the bad shell.
