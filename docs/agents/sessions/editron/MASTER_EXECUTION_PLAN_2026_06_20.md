...
| P13 Cross-overlay choreography | **PARTIAL, scheduler infrastructure wired** | `overlay-timeline-memory.ts` feeds shared pressure atoms and `cross-overlay-choreography.ts` now runs in the live unified-candidate path before final EDL stamping. It suppresses same-lane stacks, unlinked audio on crowded moments, and unlicensed text-motion clashes; kept decisions carry sync-group metadata and the report includes suppression counts by reason/family. | Remaining work is rendered proof on real projects, calibration of scheduler windows/thresholds, and broader event-level choreography scoring in Phase 0/12 evidence. |
