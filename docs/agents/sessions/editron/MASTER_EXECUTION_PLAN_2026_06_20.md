...
| --- | --- | --- | --- |
| P0 Rendered truth fixture | **PARTIAL, live metadata + async Lambda stills + scored reports wired** | Live Director calls `persistFinalPhase0LiveTruth` after final overlay save, persists `qualityReview`, `intelligence.phase0LiveTruth`, and planned artifact evidence, then dispatches `/api/internal/workers/phase0-rendered-evidence` to render sampled full/baseline stills through Remotion Lambda. The worker now builds rendered aesthetic reports, persists `intelligence.phase0RenderedStillEvidence`, `intelligence.phase0RenderedAestheticReport`, `intelligence.renderedQualityEvidence`, and a rendered quality gate; skipped/failed/partial still evidence now carries machine-readable `statusReason`, the gate reason reports `missing_rendered_evidence` instead of generic `missing_quality_review`, and the worker route exposes the reason in logs/JSON. Partial/completed still evidence without rendered-aesthetic quality evidence remains claimable for retry. | Code-level scoring and missing-evidence reason plumbing are wired; remaining work is live env proof on real projects, broader gate-policy hardening, and enough real rendered evidence before P0 becomes the hard calibration source. |
...
| P3 Caption planner | **PARTIAL** | Canonical final-timeline caption track exists, creates one caption overlay with multiple readable caption groups, and caption moment planning reads speech/readability/screen-pressure atoms. Code now consumes V-JEPA textBoxes plus semanticVisual OCR as protected regions, scores top/bottom caption geometry against those regions, and persists sourceTextProtectedRegionCount, semanticVisualOcrSegmentCount, and selectedRegion; canonical-caption-track.test.ts proves lower-third source text moves captions to the top. | Still one track container rather than a true moment-scoped caption planner/renderer ownership model. Remaining caption work is rendered proof/calibration, not re-wiring OCR avoidance. |
| P4 Visual perception / VLM cut intelligence | **PARTIAL, V-JEPA cut intelligence + semantic VU perception facts built/wired** | `video-analysis/route.ts` runs Step 1.58 before silence removal, calls V-JEPA, refines `rawFootageAnalysis.silenceRemovalPlan` through `visual-cut-intelligence.ts`, then `executeSilenceRemoval` consumes the refined plan. Focused tests prove visual dead-air removal and visual-boundary splits become real timeline video-overlay changes. `signal-registry.ts` projects `visualCutIntelligence.perception` into global visual perception signals, and VU now emits bounded `visualPerceptionWindows` that SegmentAnalysis aligns to transcript segments and projects as per-moment `visual.perception.*` signals. `UnifiedMomentContext` and `moment-bundle.ts` preserve both summary and semantic perception facts as primitive atoms; trusted V-JEPA perception still feeds shared screen context only when direct primitives are absent. | Remaining production work: calibrated thresholds, real-project rendered proof, visual-heavy/visual-only fixture coverage, and family-specific rendered placement proof of these perception facts. |
...
| P6 Transition planner | **DONE as planner infrastructure** | Producer pre-gates transition decisions at clip boundaries/pairs; transition boundary planner reads topic, pause, beat, motion, visual change, shot/subject jumps, semantic contrast, audio tail, and repetition pressure. Transition render code conserves source offsets for tile pre-roll, speed segments are source-contiguous, speed-change has an active-speech guard, and J/L cuts execute through native-audio boundary overlays; verified by time-conservation-seam.test.ts, transition-render-form.test.ts, and edl-param-contract.test.ts. | Rendered timing/choreography proof still belongs to P13/P12. Do not redo the Wave-2 time-conservation code path unless a fresh rendered project shows drift beyond the tests. |
| P7 SFX / BGM | **PARTIAL** | Atomic SFX form, sync anchors, provider candidate gate, R2/cache behavior, and strict timing validation exist; provider path is still Freesound-first and asset quality is provider-dependent. Auto-BGM dispatch exists, source-music detection now consumes project-level Essentia evidence, and auto-BGM persists why BGM was added/skipped into intelligence.autoBgmDecision + intelligence.audio.autoBgmDecision. Raw upload analysis derives native-audio speech regions from transcript words, writes hasNativeAudio + metadata.nativeAudioEvidence onto video overlays, and the sound renderer ducks BGM under those regions; native-audio-evidence.test.ts covers split clips and quality review. | Full SFX system remains: multi-provider/provider abstraction, better rejection telemetry, richer non-transition roles, and calibration of skip/place decisions. BGM still needs L3 live proof that the async worker creates the overlay when the persisted recommendation says yes. |
...
| P13 Cross-overlay choreography | **PARTIAL, scheduler infrastructure wired + Phase 0 suppression/final-bypass evidence summarized** | `overlay-timeline-memory.ts` feeds shared pressure atoms and `cross-overlay-choreography.ts` runs in the live unified-candidate path before final EDL stamping. It suppresses same-lane stacks, unlinked audio on crowded moments, and unlicensed text-motion clashes; kept decisions carry sync-group metadata; Phase 0 manifest/taxonomy now surfaces scheduler suppression/shape counts plus final-overlay bypass counts/samples for canonical captions, async audio, and post-EDL drift zoom overlays created outside the decision scheduler. | Remaining work is rendered proof on real projects, calibration of scheduler windows/thresholds, and broader pixel/audio-level event scoring in Phase 0/12 evidence. |
...
3. **Harden P4 visual perception / VLM cut intelligence** beyond the built V-JEPA cut-refinement slice: semantic VU perception + atom consumption are now built; next is real fixture/rendered proof, calibration, visual-heavy/visual-only coverage, and family-specific placement proof.
...
5. **Do not loop on stale wiring defects.** Caption OCR avoidance and transition/speed time conservation are code-verified; next work must either use rendered evidence to harden them or move to a genuinely open item.
6. **Continue P7/P9/P10/P11/Rule-11 only with rendered evidence**, not by adding new hidden menus.
7. **Run P15 calibration only after P0/P12 rendered gates are trustworthy.**
...
Current implementation note (code-verified 2026-07-04): P4 now has two wired perception layers. video-analysis/route.ts runs a pre-cut V-JEPA pass, calls refineCutPlanWithVisualIntelligence, replaces the raw silenceRemovalPlan, persists intelligence.visualCutIntelligence, and then the existing silence-removal executor applies that refined plan. This covers V-JEPA-based visual protection, visual dead-air removal, visual-boundary split actions, and a persisted visualCutIntelligence.perception summary for downstream planners (primary visual mode, subject/text/motion ratios, negative-space preference, placement trust, explainability, and missing evidence). The 2026-07-03 slices project that perception summary through SignalTimeline global signals, UnifiedMomentContext, primitive AtomicMomentBundle atoms, trusted shared screen-context fallback evidence, and existing `negative_space_*` signal-map keys. The 2026-07-04 slice adds bounded Gemini/VU `visualPerceptionWindows`, sanitizes them as perception facts only, stores them on SegmentAnalysis global context, aligns them to transcript segments as `semanticVisual`, projects them into per-moment `visual.perception.*` signals, and exposes OCR/window-count facts as primitive atoms. Direct V-JEPA primitives remain authoritative, VU does not choose edits/overlays, and degraded/semantic perception does not invent placement strength. Pause tightening is covered by active code, not a separate untracked defect: `buildSilenceRemovalPlan` emits `shorten` actions from `silenceThreshold.shortenRangeMs/shortenTargetMs`, `buildPacingSplitActions` adds non-destructive transcript-boundary splits, and `refineCutPlanWithVisualIntelligence` protects/removes/splits based on V-JEPA evidence before silence removal executes. Remaining work is calibration, real rendered proof, visual-heavy/visual-only fixture coverage, and family-specific placement proof.
