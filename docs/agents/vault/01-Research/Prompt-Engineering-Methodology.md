...
tags:
  ...
  - prompts
created: 2026-05-24
source: memory/prompt_engineering_methodology.md
proven_on: Transcript editor prompt (Gemini 3.1 Pro), F1=1.000 across 10/10 seeds
---

# Prompt Engineering Methodology

Proven process that took retake detection from 37% failure to F1=1.000 (perfect match) across 10/10 seeds. Applies to ALL 13 Gemini prompts in the Editron codebase.
...
- "When unsure, KEEP" -- better to keep something questionable than to cut good content
...
**Location:** `scripts/prompt-optimization/eval-transcript-editor.mjs`
...
# Multi-seed robustness test (seeds 1-10)
GEMINI_API_KEY=xxx node scripts/prompt-optimization/eval-transcript-editor.mjs --multi-seed
```

**Acceptance criteria:**
- All 10 seeds must score F1 > 0.90
- If min(F1) < 0.85, the prompt needs work
- Zero bad runs is the target

---

## Checklist for Every LLM Prompt

Every Gemini prompt in the codebase should get:

1. XML structure with clear section delimiters
2. Data-last ordering (input at the END)
3. Explicit rules (not few-shot examples)
4. Seed parameter set on `generationConfig`
5. Local eval harness with ground truth scoring

---

## Cross-References

- [[Rules-and-Constraints]] -- Rule 35 (LLM Prompt Engineering Methodology)
- [[APIs-Models-Keys-Costs]] -- Gemini model IDs and costs
